...
| **`F6`** | **INICIADA · EN CURSO**, por la misma resolución. Lo IMPLEMENTADO y lo todavía NO IMPLEMENTADO no se enumera aquí: su única sede es [`04-CONTRATOS-TECNICOS.md`](04-CONTRATOS-TECNICOS.md). **Iniciada no es completada, y nada de `F6` está CERTIFICADO.** **TRES verificaciones independientes lo han juzgado y NINGUNA la ha certificado**: la del 2026-09-03 y la del 2026-09-04 —ésta, el ÚNICO gate autorizado por su encargo—, las dos declaradas **NO VÁLIDAS por cobertura** y las dos con bloqueantes internos vivos; y la de `O30` del 2026-09-07, que **sí** alcanzó cobertura y **tampoco certifica**: fallan tres de las ocho condiciones de `O30` §7 —`M-04 NO SUPERADA`, `C-L.7 NO CERRADA` y dos GRAVES internas—, y cada una basta por sí sola. Su registro es [`docs/f6/07-VERIFICACION-FINAL-O30-20260907.md`](../f6/07-VERIFICACION-FINAL-O30-20260907.md). `O30` §10 cerró la recursión automática. **Y la VERIFICACIÓN POR COMPOSICIÓN de `O31`**, la intervención humana posterior sobre `V-G1`, `V-G2`, `M-04` y `C-L.7`, **tampoco certifica**: es la PRIMERA con `COBERTURA INTEGRAL` declarada —la resta `MODIFICADO − LEÍDO ÍNTEGRO` quedó VACÍA y demostrada mecánicamente— y falla en cuatro de las nueve condiciones de `O31` §7, con tres BLOQUEANTES y dos GRAVES que **no son regresiones del delta** sino puertas abiertas del aparato nuevo. Su registro es [`docs/f6/09-VERIFICACION-POR-COMPOSICION-O31-20260907.md`](../f6/09-VERIFICACION-POR-COMPOSICION-O31-20260907.md). `O31` §10 para definitivamente: lo que siga es una decisión estrictamente humana. `F6` queda **ABIERTA**, que es lo que su adjudicador declaró, y este estado **no se mueve**: un veredicto que no certifica no cierra | la resolución `O24` §2, en la [sede canónica](../owner/ADS-OWNER-RESOLUCIONES.md) · el registro del gate en [`docs/f6/03-GATE-DE-CERTIFICACION-FINAL-20260904.md`](../f6/03-GATE-DE-CERTIFICACION-FINAL-20260904.md) |
| **PesquerApp** | **BLOQUEADA.** Sin MVP, sin piloto desechable, sin adopción parcial, sin integración anticipada y sin ejecución productiva, hasta que `F6` implemente **y CERTIFIQUE** sus contratos. Ni el cierre de `F4c`, ni el de `F5`, ni el inicio de `F6` la desbloquean | resolución del Owner `O20` §8 y `O24` §4, en la [sede canónica](../owner/ADS-OWNER-RESOLUCIONES.md) |
