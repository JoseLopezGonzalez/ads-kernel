...
| **`F5`** | **AUTORIZADA DOCUMENTALMENTE, y NO INICIADA.** Autorizar no es iniciar: nada de `F5` se ha redactado, tocado ni ejecutado | ídem |
...
```text
INICIAR `F5`        es un acto del OWNER. La autorización documental existe; el arranque
                    no. Ningún documento, ningún corpus y ningún agente lo sustituye
