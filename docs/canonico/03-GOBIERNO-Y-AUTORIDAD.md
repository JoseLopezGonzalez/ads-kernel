...
| **`F5`** | **CERRADA**, por acto posterior y expreso del Owner, que declara demostradas las cuatro condiciones de `O23` §11: `F5-A`–`F5-G` completos, `A1`–`A7` satisfechos, ningún borrador presentado como aprobado sin estarlo y validación final satisfactoria. **El cierre no lo declara este corpus**: lo declara la resolución | la resolución `O24` §1, en la [sede canónica](../owner/ADS-OWNER-RESOLUCIONES.md). El acto de inicio: [`docs/f5/01-ACTO-DE-INICIO-DE-F5.md`](../f5/01-ACTO-DE-INICIO-DE-F5.md) |
| **`F6`** | **CERRADA · IMPLEMENTADA · ACEPTADA POR EL OWNER · CERTIFICACIÓN TÉCNICA `O31` NO OBTENIDA · RIESGO RESIDUAL DECLARADO.** Cerrada por **ACEPTACIÓN EXPRESA** del Owner en `O32`, **no por certificación técnica**: la verificación por composición de `O31` **NO CERTIFICÓ**, y su resultado se conserva literal y sin reinterpretar —`V-G1 NO CERRADA`, `V-G2 NO CERRADA`, `M-04 NO SUPERADA`, `C-L.7 NO CERRADA`, `F6 NO CERTIFICADA`, `F6 ABIERTA`—. Lo que `O32` §1 acepta como demostrado sobre la candidata juzgada, y lo que `O32` §3 acepta como riesgo residual, son cosas distintas y se leen por separado en su texto. El riesgo residual pasa a la familia de deuda `ADS-HARDENING`, que **no bloquea la adopción inicial** y sólo bloquea una operación concreta cuando esa operación dependa materialmente del mecanismo pendiente: su sede es [`06-DEUDA-Y-LIMITACIONES-VIGENTES.md`](06-DEUDA-Y-LIMITACIONES-VIGENTES.md). **NO se abrirán nuevos gates de `F6`** —`O32` §10—, y las mejoras de `V-G1`, `V-G2`, `M-04` y `C-L.7` son mantenimiento del ADS genérico, no condición previa de su uso. Lo IMPLEMENTADO y lo todavía NO IMPLEMENTADO no se enumera aquí: su única sede es [`04-CONTRATOS-TECNICOS.md`](04-CONTRATOS-TECNICOS.md) | la resolución `O32` §6, en la [sede canónica](../owner/ADS-OWNER-RESOLUCIONES.md) · el cierre ejecutivo en [`docs/f6/10-CIERRE-EJECUTIVO-DE-F6-O32-20260908.md`](../f6/10-CIERRE-EJECUTIVO-DE-F6-O32-20260908.md) · el registro de la verificación que no certificó, en [`docs/f6/09-VERIFICACION-POR-COMPOSICION-O31-20260907.md`](../f6/09-VERIFICACION-POR-COMPOSICION-O31-20260907.md) |
| **PesquerApp** | **AUTORIZADA PARA CREAR SU REPOSITORIO ADS DEFINITIVO · REPOSITORIO ADS DE PESQUERAPP NO CREADO · PESQUERAPP NO INICIADA BAJO ADS.** Al quedar `F6` cerrada por aceptación expresa del Owner desaparece el bloqueo previo, y **eso es todo lo que desaparece**: `O32` §7 **no ordena crear el repositorio**, que se hará por orden posterior y separada. Cuando se ejecute: **repositorio global ADS definitivo y directo**, sin piloto, sin copia temporal y sin adopción paralela desechable; independiente de los repositorios reales de frontend, backend, móvil e infraestructura; sólo para gobernar y orquestar el equipo; con el ADS genérico como fuente y **sin convertirse en un fork divergente** —`O32` §8— | la resolución `O32` §7 y §8, en la [sede canónica](../owner/ADS-OWNER-RESOLUCIONES.md) |
...
| **`C-L.7`** | **NO CERRADA.** El control de `O31` §6 deja escapar variantes de la clase: lo midió el verificador de `O31` con once nuevas y cinco mecanismos aislados por pares mínimos. `O32` **no la declara cerrada**; acepta su riesgo y la traslada a `ADS-HARDENING-04` | doc 34 §10 · el dictamen en [`docs/f6/09-VERIFICACION-POR-COMPOSICION-O31-20260907.md`](../f6/09-VERIFICACION-POR-COMPOSICION-O31-20260907.md) · la aceptación en `O32` §2 y §5 · [`06-DEUDA-Y-LIMITACIONES-VIGENTES.md`](06-DEUDA-Y-LIMITACIONES-VIGENTES.md) |
| **`M-04`** | **NO SUPERADA.** El duodécimo árbol sigue vivo: el aparato pierde un instrumento y su evidencia si además se altera la declaración que define su pertenencia. `O32` **no la declara superada**; acepta su riesgo y la traslada a `ADS-HARDENING-03` | ídem |
| **`V-G1`** | **NO CERRADA.** La completitud del universo de instrumentos admite una exclusión controlada por material interno al propio árbol. `O32` **no la declara cerrada**; acepta su riesgo y la traslada a `ADS-HARDENING-01` | el dictamen en [`docs/f6/09-VERIFICACION-POR-COMPOSICION-O31-20260907.md`](../f6/09-VERIFICACION-POR-COMPOSICION-O31-20260907.md) · la aceptación en `O32` §2 y §5 · [`06-DEUDA-Y-LIMITACIONES-VIGENTES.md`](06-DEUDA-Y-LIMITACIONES-VIGENTES.md) |
| **`V-G2`** | **NO CERRADA.** La dispensa reflexiva puede transferirse manipulando la invocación declarada. `O32` **no la declara cerrada**; acepta su riesgo y la traslada a `ADS-HARDENING-02` | ídem |
...
CERRAR `F5`         era un acto POSTERIOR y EXPRESO del OWNER, y **ya se emitió**: la
                    resolución `O24` §1 lo declara sobre las CUATRO cosas que `O23` §11
                    exigía demostrar —que `F5-A`–`F5-G` están completos · que `A1`–`A7`
                    ...
                    Su estado vigente es §6

INICIAR `F6`        exigía `F5` cerrada, y **ya se emitió**: la resolución `O24` §2 la
                    autoriza e inicia. Iniciar NO es completar: `F6` demuestra su
                    suficiencia con implementación ejecutable y pruebas reproducibles, y
                    ningún documento equivale a implementación ni a certificación
