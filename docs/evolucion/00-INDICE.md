...
| — | **PUBLICACIÓN DE LA CANDIDATA Y CORRECCIONES POSTERIORES** | `review/f4c-candidate-20260828-r2` · `comprobar_negativos.py` · `verificacion/` · `CHECKPOINT-ADS-NEXT.md` | **publicada, y NO certificada.** La candidata queda publicada en **`1b588ac`** y validada con Python 3.11.16: **13/13**, **57/57**, **67 detectadas y 0 NO detectadas**, `T158` **SUPERADA** y `T161` = **293**, con validación independiente posterior que lo confirma. Lo publicado deja de ser «nada publicado», y `kernel/operativo/` deja de estar «intacto»: hay una **excepción NOMBRADA** —`comprobar_negativos.py`, `.upstream-hash` y la evidencia derivada—, y lo normativo sigue intacto. **NADA DE ESTO ES ARQUITECTURA**: `N158g` derivaba su fixture de la cifra publicada y dependía del orden del manifiesto; ahora la deriva del corpus vigente. Y la batería propia era **inauditable** —calculaba mal la raíz y caía a una ruta codificada, comprobando el repositorio del autor desde cualquier otro clon—; ahora deriva la raíz de `__file__` y da **30/30 desde cualquier ubicación**. **`F4c` sigue ABIERTA, `F5` sigue NO AUTORIZADA, y el gate independiente sobre la candidata corregida TODAVÍA NO SE HA INICIADO** |
