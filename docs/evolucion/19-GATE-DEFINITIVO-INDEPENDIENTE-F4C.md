...
| 15 | `G-1` | GRAVE | CORREGIDO_EN_F4 | `SEG` y `CNS` obligatorias de `proceso:DEP` en `U5b` | no | no | no | no | **SUPERADA** — derivé `DEP`: obligatorias `SEG`, `CNS`, `VER`. **Y es la corrección que causa `K-02`** |
