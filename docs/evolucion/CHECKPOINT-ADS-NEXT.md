...
> **El OWNER ha resuelto la FRONTERA entre `F4c` y `F6` —es `O20`— y después la SEMÁNTICA de
> `C-L.5` —es `O21`—. `F4c` sigue ABIERTA y `F5` sigue NO AUTORIZADA: ninguna de las dos la
> cierra, y las dos lo dicen expresamente.**
>
> **CUÁLES son las resoluciones del Owner NO se escribe aquí**: se derivan de la sede, que es
> la única que las publica —`C-L.7`, y la regla de `J-07` sobre enumeraciones vivas—.
>
> ```bash
...
> **SEDE CANÓNICA**, y **no se copia aquí**. Sus proyecciones derivadas son `O20`/`D109` y
> `O21`/`D110` en
> [`DECISIONES-Y-CONTRADICCIONES.md`](../rediseno/DECISIONES-Y-CONTRADICCIONES.md) y §15.4 y
> §15.8 del documento 11, y el contrato ejecutable de `O20` es **§20 del documento 11**.
>
...
> la MATRIZ DE CIERRE de este fichero. **El reparto NO se escribe aquí** —`H-16`: esta misma
> cabecera ordena derivar los cardinales, y estas líneas copiaban «catorce» y «ocho» al lado
> de la tabla que los da, que es la forma exacta de `J-07`—. Sale de la tabla:
>
> ```bash
> awk '/^\| `C-[0-9]+` \|/{print $4}' docs/evolucion/CHECKPOINT-ADS-NEXT.md | sort | uniq -c
>
> # y los que bloquean `F6` **y** PesquerApp, que tampoco se cuentan a mano:
> awk '/^\| `C-[0-9]+` \|/' docs/evolucion/CHECKPOINT-ADS-NEXT.md |
>   grep -c '| \*\*SÍ\*\* | \*\*SÍ\*\* |'
> ```
>
> Ninguno se declara SUPERADO, y `O20` lo prohíbe expresamente mientras no se implemente y se
> ejecute en `F6`. **Los que permiten un falso verde bloquean `F6` y bloquean PesquerApp**, y
> así consta en su fila.
>
...
>
> Está en
> [`27-SEXTO-GATE-DE-CERTIFICACION-F4C.md`](27-SEXTO-GATE-DE-CERTIFICACION-F4C.md), con los
> tres dictámenes íntegros y sus SHA-256. **El recuento y la clasificación NO se copian
> aquí**: viven en su §5.
>
> **EL NOVENO ÁRBOL, y no estaba en el perímetro: estaba en el REMEDIO DEL GATE ANTERIOR.**
> `DD-02` hizo que la guarda de admisión se evaluara sobre el CONTENIDO DEL COMMIT **para
> `docs/owner/` y sólo para esa zona**; en todo el resto del corpus gobernado seguía mirando
> únicamente lo que aún no estaba en `HEAD`. Con eso, `git add -A && git commit` **sin un
> solo flag** añadía una segunda sede normativa que declaraba `F4c` cerrada y `F5`
> autorizada, dejaba `git status` vacío, pasaba **38/38**, no entraba en el universo y
> **producía el digest anclado bit a bit**. Lo encontró un revisor y **lo reprodujo el
> adjudicador por su cuenta, dos veces, en un clon desechable**.
>
> **LA CIRCULARIDAD SE HA MOVIDO POR QUINTA VEZ** —`HEAD`, emisor, entrega, perímetro, y hoy
> la guarda de admisión—, y `EE` la nombra sin cerrarla: *«cada remedio cerró el punto donde
> la circularidad estaba y no la propiedad que la produce: que la definición de QUÉ se
> verifica y de QUÉ se admite sean objetos del árbol verificado»*. Esa propiedad es lo que
> `O18` contrata para `F6` como `C`; **su manifestación de hoy es `A`**, y por eso cuenta.
>
> **NINGUNA DECISIÓN VUELVE AL OWNER**, por tercera vez consecutiva: cuatro candidatos a
> clase `B` examinados y los cuatro caen. Ninguno de los diecinueve exige arquitectura nueva.
>
> **Y consta a favor, porque es verdad:** `DD-17` quedó **ROTO por primera vez en seis
> gates** —el commit del manifiesto deja el árbol en 13/13 con `T147` SUPERADA y cero
> evidencia sucia, en los dos árboles—; `DD-01` **cerró su clase** y resistió cinco
> variantes; las dos restas de cobertura cierran a **∅** y `EE` lo dice primero; `X63` no se
> presenta como prueba ejecutada en ninguna de sus ocho sedes; y `EE` declara expresamente
> que **el veredicto NO falla por la tanda: el defecto está en el ALCANCE de un remedio, no
> en su ejecución**.

> **[ESTADO ANTERIOR · antes del SEXTO GATE DE CERTIFICACIÓN, documento 27. Se conserva
> y NO describe el estado vigente.]**
> **El QUINTO GATE DE CERTIFICACIÓN devolvió `INSUFICIENTE PARA F5`, y —al revés que el
> cuarto— EL GATE ES VÁLIDO. `F4c` sigue ABIERTA y `F5` sigue NO AUTORIZADA.**
>
> Está en
> [`26-QUINTO-GATE-DE-CERTIFICACION-F4C.md`](26-QUINTO-GATE-DE-CERTIFICACION-F4C.md), con los
> tres dictámenes íntegros y sus SHA-256 publicados. **El recuento y la clasificación NO se
> copian aquí**: viven en su §5, y quien los quiera los lee ahí.
>
> **POR QUÉ ES VÁLIDO, Y ES LO PRIMERO QUE HAY QUE DECIR.** El cuarto murió porque el
> coordinador **transcribió el sobre a mano** y las cinco transcripciones difirieron en ocho
> campos. Desde este gate **el sobre no se transcribe**: se emite UNA vez a un fichero fuera
> del repositorio auditado y cada revisor lo lee de ahí. `DD` verificó que los cuatro bloques
> embebidos son byte a byte el fichero. **Divergencia cero.** La causa exacta de la invalidez
> anterior queda cerrada **por construcción, no por diligencia**.
>
> **LO DECISIVO ES DEL PROPIO ADJUDICADOR, Y NO DE LOS REVISORES: EL OCTAVO ÁRBOL.** El
> perímetro de exclusión decía excluir «por NATURALEZA» y excluía **por NOMBRE**: un documento
> que declara `F4c` cerrada y `F5` autorizada, nombrado `<algo>.pyc` bajo `docs/owner/`,
> alcanzaba el commit, dejaba `git status` vacío, pasaba la batería y el runner enteros, **no
> entraba en el universo, no recibía fila ni revisor y producía el digest del sobre bit a bit
> idéntico**. La circularidad **se ha movido por cuarta vez, y hoy estaba en el PERÍMETRO**.
>
> **Y `BB4` nombra la clase, que es la frase que este expediente necesitaba:** *el sistema
> cierra INSTANCIAS y no CLASES* — la corrección se aplica con la forma sintáctica exacta del
> contraejemplo, y el defecto reaparece una sede más allá; en un caso, **quince líneas por
> encima de la nota que lo corrige**.
>
> **NINGUNA DECISIÓN VUELVE AL OWNER**, por segunda vez consecutiva: `DD` examinó cuatro
> candidatos a clase `B` y los cuatro caen. Ninguno de los hallazgos es BLOQUEANTE y ninguno
> exige arquitectura nueva. **«Ésta sigue siendo una candidata sólida, y el trabajo debe
> SEGUIR.»**
>
> **Y lo que SÍ quedó cerrado en la candidata:** `AA-01` CERRADO y GENERALIZA —`docs/owner/`
> se barre entero— · `AA-05` CERRADO —el sobre publica un NOMBRE y no un rol— · el gate NO
> tocó `verificacion/` tras publicar el manifiesto · las 74 filas del manifiesto casan sin una
> discrepancia · la fila del propio derivador es idéntica en los dos árboles · los contratos
> de prueba de `F6` —`X55`, `X58`, `W8`, `W11`, `W17`— están CORRECTOS · el diseño NO ha
> cambiado y `O19` sigue implementado **sin escribir una sola protección interna nueva**.

> **[ESTADO ANTERIOR · antes del QUINTO GATE DE CERTIFICACIÓN, documento 26. Se conserva
> y NO describe el estado vigente.]**
...
> precondición propia de esa ejecución.** Su motivo, **RESUMIDO por este registro y NO en
> sus palabras** (`DD-06`): robustez y revalidación permanente por encima del ahorro
> operativo. **Sus palabras, que son lo único entrecomillado y cuya sede es
> [`docs/owner/ADS-OWNER-RESOLUCIONES.md`](../owner/ADS-OWNER-RESOLUCIONES.md), `O17` ·
> Texto:** — «*No elijo la alternativa
...
regla_de_reanclaje: ESTE BLOQUE ES EL ESTADO REANUDABLE y va SIN rótulo histórico: describe el
             ...
             acuerde, y en la forma que S-16 y S-17 ya establecieron para este fichero:
               ...
               2  EL ÚLTIMO GATE Y SU DOCUMENTO NO SE ESCRIBEN A MANO. Se derivan con
                  ...
                  su única sede
               3  LA ÚLTIMA RESOLUCIÓN DEL OWNER Y LA ÚLTIMA DECISIÓN TAMPOCO. **Y la del
                  Owner se deriva de SU SEDE CANÓNICA, no del registro derivado** —desde O19
                  la sección 2 de DECISIONES es PROYECCIÓN, y derivar el estado del Owner de
                  una proyección es leer la autoridad en la copia—:
                  ...
                  Y la lista ENTERA de resoluciones, cuando haga falta, sale de la misma sede:
                       grep -oE '^# `O[0-9]+`' docs/owner/ADS-OWNER-RESOLUCIONES.md
...
metodo_anterior: SIS/Evolucion · ÚLTIMO GATE INDEPENDIENTE DEVUELTO —GATE VÁLIDO, `C-L.5`
             ...
             EL MÉTODO DE CORRECCIÓN ITERATIVA SIGUE DETENIDO: la OPCIÓN C se activó tras el
             gate anterior y **este gate no la levanta**. Lo que hubo entre medias fue una
             TANDA DE IMPLEMENTACIÓN ACOTADA, autorizada expresamente y con alcance cerrado,
             seguida de esta VERIFICACIÓN EXTERNA. No se ha abierto ningún ciclo.
             LO QUE ESTE GATE ESTRENA, y no había ocurrido en once: **su adjudicador CERTIFICA
             LA COBERTURA midiendo las SEIS condiciones de `O21` §4 una a una, MIENTRAS
             devuelve insuficiencia**, y publica la prueba contrafáctica en las DOS
             direcciones de que cada declaración es invariante ante la otra. Es exactamente
             lo que `O21` vino a hacer posible, y es la primera vez que se ejerce.
             EL ORDINAL Y EL DOCUMENTO DEL GATE NO SE ESCRIBEN AQUÍ —regla 2—:
                 ls docs/evolucion/[0-9][0-9]-*.md | sort | tail -1
             EL VEREDICTO, EL RECUENTO, EL CIERRE INDIVIDUAL DE CADA HALLAZGO y LA TUPLA de
             la certificación viven DENTRO de ese documento, que es inmutable y es su única
             sede. Lo que este bloque sí dice, porque es el estado y no una cifra: **F4c
             sigue ABIERTA y F5 sigue NO AUTORIZADA**. APLICAR NO ES CERTIFICAR, DETENER NO
             ES CERRAR, y CERTIFICAR COBERTURA NO ES DECLARAR SUFICIENCIA.
             ...
             cobertura y devolver insuficiencia son actos DISTINTOS e INDEPENDIENTES, y el
             último adjudicador los emitió por separado. Ni este campo ni ningún otro deduce
             uno del otro.
metodo_anterior: SIS/Evolucion · ÚLTIMO GATE INDEPENDIENTE DEVUELTO —GATE VÁLIDO, `C-L.5`
...
metodo_anterior: SIS/Evolucion · RESOLUCIÓN `O21` DEL OWNER REGISTRADA EN LA SEDE CANÓNICA
             ...
             necesite lo lee ahí.
             El veredicto, el recuento y la clasificación del último gate viven DENTRO de su
             documento, que es su única sede e inmutable. Lo que este bloque sí dice, porque
             es el estado y no una cifra: **F4c sigue ABIERTA y F5 sigue NO AUTORIZADA**.
             EL ESTADO DE CADA CONDICIÓN `C-L` NO SE ESCRIBE AQUÍ, y es `C-06` del OCTAVO
             ...
             lee, no se copia:
                 grep -n 'CLASIFICACIÓN VIGENTE' docs/evolucion/CHECKPOINT-ADS-NEXT.md
             Y DESDE `O21`, EL ESTADO DE `C-L.5` SE LEE CON SU REGLA NUEVA: certificar
             cobertura y devolver insuficiencia son actos DISTINTOS e INDEPENDIENTES, y
             pueden coexistir en el mismo gate. Ni este campo ni ningún otro de este bloque
             deduce uno del otro.
...
metodo_anterior: SIS/Evolucion · RESOLUCIÓN `O20` DEL OWNER REGISTRADA Y PROPAGADA COMO
             `D109` —la FRONTERA entre `F4c` y `F6`—, con el CONTRATO OBLIGATORIO DE `F6`
             escrito en §20 del documento 11 y los 22 hallazgos del octavo gate repartidos
             en la MATRIZ DE CIERRE. APLICADA, NO CERTIFICADA · F4c ABIERTA · F5 NO
             AUTORIZADA
metodo_anterior: SIS/Evolucion · ÚLTIMO GATE ARQUITECTÓNICO EJECUTADO —el PRIMERO bajo
             `O20`, tres agentes de contexto limpio, dos revisores en paralelo sin verse y
             un adjudicador— · VEREDICTO INSUFICIENTE PARA F5 · GATE VÁLIDO · su recuento,
             sus severidades y su clasificación viven EN SU DOCUMENTO, que es su única sede
...
metodo_anterior: SIS/Evolucion · RESOLUCIÓN O17 DEL OWNER RECIBIDA Y EN PROPAGACIÓN, y TANDA
             ...
             MACROCIRCUITO AL ARRANCAR, como precondición propia. Motivo, RESUMIDO por
             este registro y NO en palabras del Owner (`DD-06`): robustez y revalidación
             permanente por encima del ahorro operativo — el literal está en
             `docs/owner/ADS-OWNER-RESOLUCIONES.md`, `O17` · Texto. La clase B queda
...
based_on:    LA BASE VIGENTE NO SE NOMBRA AQUÍ, y es `C-08` del OCTAVO GATE: este campo
             ...
             tres eventos más allá. Se retira: la base es el commit en curso y su SHA no se
             escribe —regla 1—:
                 git rev-parse HEAD
                 git log --oneline -1
             `H-09` DEL GATE ARQUITECTÓNICO FINAL, y era la MISMA CLASE otra vez, dos veces
             en el mismo campo. (i) Detrás de la retirada quedaba un FRAGMENTO HUÉRFANO
             —«Son INMUTABLES, y el VEREDICTO y el RECUENTO de cada gate viven EN CADA
             DOCUMENTO, no aquí. La»— cortado a media frase por la corrección anterior, que
             `EE-04` reancló y `S2-03` retiró a medias. (ii) Y más abajo colgaba la SEGUNDA
             COLUMNA de una fila cuya primera columna ya no existía —«dictámenes Y y Z,
             adjudica AA · GATE INVÁLIDO · VEREDICTO INSUFICIENTE · C-L.5 pasa a ABIERTA»—:
             un ESTADO COPIADO, dentro del campo que declara no copiar estados, atribuido a
             un documento que ya nadie podía identificar. Los dos se RETIRAN.
             LAS FUENTES DE LA BASE, POR SU SEDE Y SIN ENUMERAR SU CONTENIDO. Cada una dice
             DE QUÉ responde; **qué identificadores contiene lo dice ELLA**, no este campo:
                 docs/owner/ADS-OWNER-RESOLUCIONES.md
                                     LA SEDE CANÓNICA DE LAS RESOLUCIONES DEL OWNER, creada
                                     por O19. ES FUENTE DE AUTORIDAD y NO se deriva de los
                                     documentos numerados: la sede no es uno de ellos. Es
                                     APPEND-ONLY, el índice la enlaza y G-29 la custodia
                                     contra el COMMIT QUE LA CREÓ. Desde O19, toda sede
                                     derivada que cite una resolución del Owner cita DE AHÍ
                                     y no de una paráfrasis. QUÉ RESOLUCIONES CONTIENE se
                                     deriva, y no se escribe aquí:
                                         grep -oE '^# `O[0-9]+`' \
                                           docs/owner/ADS-OWNER-RESOLUCIONES.md
                 docs/evolucion/[0-9][0-9]-*.md
                                     LOS DOCUMENTOS NUMERADOS. NO SE ENUMERAN AQUÍ, y es
                                     S2-03 del SÉPTIMO GATE: la lista se conservaba «por
                                     comodidad de lectura» y caducaba en cada gate.
                                     Reanclarla añadiendo los que faltan cierra la INSTANCIA
                                     y deja viva la CLASE. Se remite al comando de la regla
                                     2 de `regla_de_reanclaje`:
                                         ls docs/evolucion/[0-9][0-9]-*.md | sort
                                     Son INMUTABLES, y G-22 los custodia contra HEAD y
                                     contra la revisión base. EL VEREDICTO, EL RECUENTO, LAS
                                     SEVERIDADES y LA CLASIFICACIÓN de cada gate viven EN SU
                                     PROPIO DOCUMENTO: aquí se REMITE y no se copia, por la
                                     regla 1
                 docs/evolucion/verificacion/CORRIGENDUM-DICTAMENES-INMUTABLES.md
                                     los errores de hecho señalados SIN tocar los dictámenes
                 docs/rediseno/DECISIONES-Y-CONTRADICCIONES.md
                                     el REGISTRO DE DECISIONES, que desde O19 es PROYECCIÓN
                                     DERIVADA de la sede del Owner y no fuente de autoridad
                                     sobre ella. QUÉ contiene tampoco se enumera aquí —lo
                                     hizo durante siete tandas y caducó en todas—:
                                         grep -oE '^\| `D[0-9]+` \|' \
                                           docs/rediseno/DECISIONES-Y-CONTRADICCIONES.md
                                         grep -oE '^### `O[0-9]+`' \
                                           docs/rediseno/DECISIONES-Y-CONTRADICCIONES.md
                 docs/evolucion/11-ARQUITECTURA-INTEGRADA.md
                                     la ARQUITECTURA de F4, y desde D109 el contrato
                                     ejecutable del verificador de F6 —§20—, su
                                     clasificación semántica —§20.3— y el orden de
                                     construcción con su nodo 9 —§18—
...
last_meaningful_event_anterior: UN GATE INDEPENDIENTE VERIFICA LA TANDA QUE APLICÓ LOS HALLAZGOS
             ...
             **INSUFICIENTE PARA F5**. Su ordinal y su documento NO se escriben —regla 2—: se
             ...
             individual de cada hallazgo y la TUPLA de la certificación viven DENTRO de él.
             LO QUE ESTRENA: **por primera vez en el expediente un adjudicador emite las DOS
             declaraciones POR SEPARADO Y CERTIFICA LA COBERTURA**, midiendo las seis
             condiciones de `O21` §4 una a una y declarándolas SATISFECHAS, **mientras
             devuelve insuficiencia**; y publica la prueba contrafáctica en las DOS
             direcciones de que cada declaración es invariante ante la otra. `O21` dejó de
             ser una norma escrita y pasó a ser una norma ejercida.
             Y por primera vez también: **los DOS revisores cierran su cobertura con
             `ASIGNADO − LEÍDO` = ∅**, con rangos enumerados y sin hueco sobre los dos
             ficheros grandes, declarando contra su propio interés qué leyeron fuera de lote
             sin contarlo y qué no pueden comprobar.
             POR QUÉ EL VEREDICTO ES INSUFICIENTE: **un hallazgo del gate anterior quedó
             FALLIDO** —una de sus dos sedes editables sigue byte a byte como estaba, y dos
             sedes vivas afirman que se corrigió—. **El recuento, el reparto y la severidad
             NO se copian aquí** —regla 1—: viven en el documento del gate.
             LO QUE ESTE GATE **NO** HACE: no cierra `F4c`, que sigue ABIERTA; no autoriza
             `F5`, `F6` ni PesquerApp, que sigue BLOQUEADA; no declara SUPERADO ningún
             hallazgo; no revisa ninguna resolución del Owner; **y no levanta la OPCIÓN C**,
             que sigue activada. Un gate no corrige, y éste no ha corregido nada.
             REANCLADO EN EL MISMO COMMIT QUE REGISTRA EL EVENTO —regla 4—, y lo anterior NO
             se borra: baja íntegro a `last_meaningful_event_anterior` —regla 5—.
last_meaningful_event_anterior: EL ÚLTIMO GATE INDEPENDIENTE DEVUELVE INSUFICIENTE PARA F5, SE
...
last_meaningful_event_anterior: EL OWNER RESUELVE LA SEMÁNTICA DE `C-L.5`, y es `O21`. La toma por
             ...
             CERTIFICADA—, con lo que cumplir sus seis condiciones no obligaba a nada, y dos
             adjudicadores consecutivos actuaron de forma OPUESTA sin incumplir norma alguna:
             uno certificó la cobertura MIENTRAS devolvía insuficiencia, el siguiente se negó
             POR devolver insuficiencia. Desde `O21`: `C-L.5` certifica COBERTURA y nada más;
             su resultado es INDEPENDIENTE del veredicto de suficiencia; NO es discrecional
             —cumplidas las seis condiciones el adjudicador DEBE certificar, fallada
             cualquiera DEBE declararla ABIERTA nombrando la condición—; la certificación
             queda ligada a una TUPLA exacta y NO se transfiere a otra candidata ni a otro
             gate; las dos declaraciones pueden COEXISTIR; y un adjudicador NO puede negarse
             a certificar cobertura por haber encontrado otros defectos. `O21` NO revisa las
             seis condiciones, NO se aplica hacia atrás —sobre el último gate se registra el
             hecho y nada más—, NO declara suficiente a `F4c`, NO corrige ningún hallazgo y
             NO autoriza `F5`, `F6` ni PesquerApp. Se propaga como `D110`, y su texto íntegro
             está en la SEDE CANÓNICA, que no se copia aquí:
                 awk '/^# /{p = ($0 ~ /^# `O21`/)} p' docs/owner/ADS-OWNER-RESOLUCIONES.md
             REANCLADO EN EL MISMO COMMIT QUE REGISTRA EL EVENTO —regla 4—, y lo anterior NO
             se borra: baja íntegro a `last_meaningful_event_anterior` —regla 5—.
last_meaningful_event_anterior: EL OWNER RESUELVE LA FRONTERA ENTRE `F4c` Y `F6`, y es `O20`. La
...
last_meaningful_event_anterior: EL ÚLTIMO GATE DE CERTIFICACIÓN DEVUELVE INSUFICIENTE PARA F5 Y
             ...
             solo flag— añadía una segunda sede normativa declarando F4c cerrada y F5
             autorizada, dejaba git status vacío, pasaba la batería entera, no entraba en
             el universo y producía el digest anclado bit a bit. DD-02 lo había cerrado
             para UNA zona: instancia cerrada, clase abierta. La circularidad se ha movido
             por QUINTA vez —HEAD, emisor, entrega, perímetro, y hoy la guarda de admisión,
             que es el remedio del gate anterior—. NINGUNA decisión vuelve al Owner, por
             TERCERA vez consecutiva. Y consta a favor: DD-17 quedó ROTO por primera vez en
             seis gates. El recuento, las severidades y la clasificación NO se copian aquí
             —regla 1—: viven en el documento del gate.
last_meaningful_event_anterior: EL CUARTO GATE DE CERTIFICACIÓN DEVUELVE INSUFICIENTE PARA F5 **Y SU
...
last_meaningful_event_anterior: EL OWNER RESPONDE la única decisión de clase B del documento 22, y su
             ...
             ejecución. Su motivo, RESUMIDO por este registro y NO en sus palabras
             (`DD-06`), es robustez y revalidación permanente por encima del ahorro
             operativo —el literal vive en `docs/owner/ADS-OWNER-RESOLUCIONES.md`, `O17` ·
             Texto—: descarta (a) porque un producto ya instalado
...
owner_captado: LA AUTORIDAD CANÓNICA DE LO QUE EL OWNER HA RESUELTO NO ES ESTE CAMPO, Y DESDE
             ...
             docs/owner/ADS-OWNER-RESOLUCIONES.md, donde cada resolución vive con su texto
             íntegro. Este campo NO las transcribe: remite.
             CUÁLES SON NO SE ESCRIBE AQUÍ, y es `H-10` del GATE ARQUITECTÓNICO FINAL: este
             campo decía «O17, O18 y O19» y «NINGUNA de las tres», y la sede ya publicaba
             más. Un cardinal escrito al lado de una sede que crece caduca en silencio, y
             éste caducó dos veces en dos tandas. Se DERIVA:
                 grep -oE '^# `O[0-9]+`' docs/owner/ADS-OWNER-RESOLUCIONES.md
...
pregunta_pendiente: NINGUNA, y no es una afirmación de este campo: **ningún gate ha elevado
             una decisión al Owner desde que existe esta cadena** —todos declararon clase B
             vacía—, y las dos últimas resoluciones las tomó el Owner POR SU CUENTA.
             CUÁL FUE LA ÚLTIMA RESPUESTA NO SE ESCRIBE AQUÍ, y es la clase de `C-L.7`: este
             campo nombraba «O19» como la última, y para cuando se leyó ya había dos
             posteriores. Se retira y se remite —regla 3—:
                 grep -o '^# `O[0-9]*`' docs/owner/ADS-OWNER-RESOLUCIONES.md | tail -1
             Lo histórico, que no caduca porque describe hechos cerrados: la RATIFICACIÓN que
             el TERCER GATE elevó la respondió O19 —ratifica el texto amplio de O18 y ordena
             su sede canónica—, y LA RATIFICACIÓN QUEDA CERRADA: O18 no vuelve a someterse a
             elección. Antes, la clase B del documento 22 —el nivel ESTRUCTURAL y su
             productor— la respondió O17 con la alternativa (b).
             **NINGUNA resolución del Owner autoriza iniciar F5**, y cada una lo declara en
             su propio texto. Ese texto no se copia aquí: la sede es
             docs/owner/ADS-OWNER-RESOLUCIONES.md.
             ...
             condición:
               ...
               PARA ÉL
                                         **MIENTRAS devuelve INSUFICIENCIA**. Es la primera
                                         vez en el expediente que las dos declaraciones se
                                         emiten por separado con prueba contrafáctica en
                                         las DOS direcciones, y es exactamente lo que `O21`
                                         vino a hacer posible. **Queda ligada a la TUPLA de
                                         seis elementos de `O21` §5 y NO se transfiere** a
                                         otra candidata ni a otro gate. El detalle está en
                                         su fila, y sólo un adjudicador la mueve
             ...
             LAS TRECE FILAS DE DETALLE, que son la sede canónica de esta clasificación:
               ...
               C-L.5  CERTIFICADA · **POR COBERTURA, PARA EL ÚLTIMO GATE Y SÓLO PARA ÉL.**
                      ...
                      **NO ES UN ACTO DISCRECIONAL, y él lo dice**: cumplidas las seis,
                      `O21` §3 le OBLIGA a certificar, y `O21` §8 le prohíbe negarse por
                      haber encontrado otros defectos. **Certifica MIENTRAS devuelve
                      INSUFICIENCIA**, y publica la prueba contrafáctica en las DOS
                      direcciones: si el hallazgo vivo estuviera cerrado la cobertura
                      seguiría certificada y el veredicto sería suficiente; si un revisor
                      hubiera dejado un tramo sin abrir la cobertura se abriría y el
                      veredicto seguiría siendo insuficiente. **Cada declaración es
                      ...
                      **[HISTÓRICO · el motivo que este renglón dio entre el documento 31 y
                      el 32]** «ABIERTA por la CONDICIÓN 5, `ASIGNADO − LEÍDO` no vacío, por
                      cinco fuentes del lote de un revisor que él mismo declaró no haber
                      leído íntegras.» Fue cierto sobre `f232d1a`; el gate siguiente midió
                      la resta VACÍA en los tres lotes. Se conserva y no se borra.
                      **[HISTÓRICO · y el motivo anterior a ése.]** Por la CONDICIÓN 5, nombrada por el adjudicador del ÚLTIMO
                      ...
                      **[HISTÓRICO · y el motivo que dio antes de eso.]** Sobre la
                      ...
                      CIERRA y NO es razón de insuficiencia**. Está ABIERTA porque **no
                      ejecutó el ACTO de certificarla**, y lo dijo con todas las letras:
                      certificar es un acto sobre el gate y él no lo emitía mientras el gate
                      devolviera insuficiencia. **La medición se cumple; el acto falta.**
                      **Y NINGUNA NORMA LE OBLIGABA**, que es lo que `O21` corrige: la regla
                      de cierre de `C-L.5` es una EXCLUSIÓN de una sola dirección —dice
                      cuándo la cobertura queda EXCLUIDA, no cuándo queda CERTIFICADA—, de
                      modo que dos adjudicadores consecutivos pudieron actuar de forma
                      OPUESTA sin incumplir nada: `FF` certificó MIENTRAS devolvía
                      insuficiencia; `HH` se negó POR devolverla.
                      **DESDE `O21`, Y SÓLO HACIA ADELANTE:** cumplidas las seis condiciones
                      sobre un gate válido, el adjudicador **DEBE** declarar
                      `C-L.5 CERTIFICADA PARA ESTE GATE`; fallada cualquiera, **DEBE**
                      declararla `ABIERTA` **nombrando la condición incumplida**. Certificar
                      cobertura y declarar suficiencia son actos INDEPENDIENTES y pueden
                      COEXISTIR; un adjudicador **no puede negarse** a certificar cobertura
                      por haber encontrado otros defectos; y la certificación queda ligada a
                      una TUPLA exacta —tree SHA, commit, SHA del manifiesto de asignación,
                      SHA de los manifiestos de lectura, identificador del gate e identidad
                      del adjudicador— y **NO se transfiere**. `O21` **NO se aplica hacia
                      atrás**: sobre el último gate se registra el hecho —condiciones
                      satisfechas, acto no emitido por ausencia de regla entonces vigente— y
                      nada más, y **ni `FF` ni `HH` incumplieron norma alguna**.
                      **NADA de esto la cierra ni la abre por decisión de este registro:**
                      certificar es un ACTO de un adjudicador, y aquí sólo se recoge el
                      último que existe. Su texto íntegro está en la sede canónica:
                          awk '/^# /{p = ($0 ~ /^# `O21`/)} p' \
                            docs/owner/ADS-OWNER-RESOLUCIONES.md
                      **[HISTÓRICO · el motivo que este renglón daba, y que describía OTRA
                      candidata]** «La REABRE el adjudicador `GG` del OCTAVO GATE por
                      ...
                      la unidad—; es el ÚLTIMO ACTO VÁLIDO de cobertura del expediente.» Fue
                      cierto sobre `61492c1`; el gate siguiente midió otra cosa sobre otra
                      candidata, y el motivo dejó de describir el objeto vigente sin que
                      nadie lo moviera. Se conserva y no se borra.
                      ...
                      **[HISTÓRICO · `EE-05` del SEXTO GATE, y es `AA-02` VIVO en la fila
                      gemela de aquella que lo declaró cerrado.]** Este renglón decía:
                      *«MIENTRAS el primero devuelva 0, `G-16` dará ROJO sobre esta
                      clasificación, y ese rojo es VERDADERO … el remedio vive en
                      `docs/evolucion/verificacion/`, que este registro NO escribe»*. **El
                      antecedente es FALSO desde `8c9ca9c`**: el instrumento ya conoce
                      «ABIERTA», el primer comando NO devuelve 0 y `G-16` está en VERDE.
                      Es la misma clase que `AA-02` castigó en la fila `C-L.7` —una
                      afirmación rancia redactada en forma de autocrítica, que cobra crédito
                      de quien la lee—, y sobrevivió **en la fila de al lado**.
                      **LO QUE ES VERDAD HOY, y NO se escribe: se publica el comando que lo
                      decide, para que no pueda volver a caducar.**
                          python3 docs/evolucion/verificacion/comprobar-correccion-gate-de-cierre.py | grep G-16
                      ...
                      **Y NO escribió «CERTIFICADA», y este registro no lo escribe por él.**
                      Certificar es un ACTO del adjudicador, no una consecuencia que el
                      coordinador deduzca de sus cifras: poner aquí «CERTIFICADA» sería
                      exactamente lo que `C-L.5` existe para impedir —una sede derivada
                      declarando cerrado lo que sólo un gate cierra— y sería la clase que
                      `Q-06` y `AA-02` castigaron. **La condición queda ABIERTA, con la
                      medición registrada**, y **el gate siguiente tiene que resolverlo
                      EXPRESAMENTE**: o emite la palabra, o dice por qué no la emite. Va
                      escrito en «Siguiente acción exacta» como pregunta suya.
                      **Y hay una razón de fondo que `BB4` encontró y nadie había visto:**
                      cerrar la resta a cero **sobre el árbol del GATE** exigiría que el
                      manifiesto contuviera su propio SHA-256, y eso es **inalcanzable por
                      punto fijo** — `DD-19`, cuya regla vive en «El criterio del gate
                      siguiente» de este mismo fichero
               ...
               C-L.7  NO CERRADA · **la MUEVE aquí el TERCER GATE DE CERTIFICACIÓN,
                      ...
                      **LO QUE ESTA TANDA HA HECHO, Y ES DE CLASE Y NO DE INSTANCIA.** El
                      ÚLTIMO gate encontró TRES instancias vivas a la vez —`H-09`, `H-10` y
                      `H-16`— y las tres son la misma: un campo que copia dentro del bloque
                      lo que el bloque declara no copiar. Corregir esas tres habría cerrado
                      otra vez la instancia y dejado la clase, que es lo que este expediente
                      lleva NUEVE gates persiguiendo. Se ha hecho lo otro:
                        · el CONJUNTO DE CAMPOS se DERIVA del propio bloque y se recorre
                          entero, no los tres señalados — el comando está en la regla 7;
                        · a cada campo se le hacen las TRES preguntas de la regla 7: ¿copia
                          un recuento, un ordinal, un estado o una enumeración que otra sede
                          publica? ¿queda un FRAGMENTO de una retirada anterior sin su
                          frase? ¿lo histórico está ROTULADO como histórico?;
                        · la GARANTÍA queda escrita DENTRO del bloque como **regla 7**, de
                          modo que no depende de que nadie se acuerde en la tanda siguiente;
                        · y la regla 3 pasa a derivar la última resolución del Owner **de su
                          SEDE CANÓNICA** y no del registro derivado, que era leer la
                          autoridad en la copia.
                      **Esto tampoco la cierra.** Sigue NO CERRADA hasta que un gate
                      independiente posterior lo juzgue, y este registro no se lo adelanta.
                      ...
                      **LO QUE ES VERDAD HOY, ejecutado sobre este árbol y no supuesto:**
                      ...
                      condición.
                      **[HISTÓRICO · `EE-05` del SEXTO GATE.]** Aquí seguía: *«Lo que hoy la
                      pone en rojo es OTRA cosa, y es la fila de `C-L.5`: el vocabulario no
                      conoce ABIERTA»*. **Ya no la pone en rojo NADA**: el vocabulario
                      conoce «ABIERTA» desde `8c9ca9c` y `G-16` está en verde. Esta fila
                      remitía a una aserción de la fila gemela que había caducado, de modo
                      que el defecto se sostenía a sí mismo entre las dos. **Se retiran las
                      ...
                      **[HISTÓRICO] Tras el QUINTO GATE, y con una recurrencia más:**
                      ...
                      con el puntero además invertido: mandaba buscar la clasificación
                      vigente «más abajo» y está ARRIBA. **Tres defectos en un renglón, y
                      SEXTA recurrencia de la clase que esta condición existe para cerrar.**
                      La tanda posterior al documento 26 lo corrige **retirando los dos
                      estados en vez de sustituirlos**, que es lo único que no vuelve a
                      caducar, y enderezando el puntero. **Reanclar no es certificar**: sólo
                      un gate independiente posterior puede devolver `C-L.7` a CERRADA
...
falta_para_cerrar_la_capa:
  ...
  · NADA PROBADO: las filas de la tabla adversarial de §2.6.7, las ventanas de caída
    ...
    **NINGÚN CARDINAL SE ESCRIBE AQUÍ, y es `EE-07` del SEXTO GATE.** Este renglón decía
    ...
    TANDA— sobrevivió aquí, una sede más allá de la corregida. **No se sustituye por otro**,
    que es lo que este corpus prohíbe desde `J-07` y lo que ya falló tres veces: se retira
    y se remite. Cada familia lleva su cifra en SU sede, y quien la necesite la deriva:
...
F4c CRÍTICA INDEPENDIENTE    ABIERTA. La cadena completa, EMITIDA por revisores y auditores
                             ...
                             SUS TRECE CONDICIONES C-L.1–C-L.13 están APLICADAS. **Cuántas
                             están cerradas o registradas NO se escribe aquí, y el estado
                             de C-L.5 tampoco**: los da la CLASIFICACIÓN VIGENTE de este
                             mismo fichero, que está **MÁS ARRIBA** —bajo el rótulo «CÓMO
                             QUEDA CADA CONDICIÓN — CLASIFICACIÓN VIGENTE», y termina en
                             «FIN DE LA CLASIFICACIÓN VIGENTE»—. Se lee, no se copia:
                                 grep -n 'CLASIFICACIÓN VIGENTE' \
                                   docs/evolucion/CHECKPOINT-ADS-NEXT.md
                             **APLICADAS NO ES CERTIFICADAS.**
                             **CORREGIDO por `DD-08` del QUINTO GATE, y eran TRES defectos
                             en un renglón que declaraba no copiar.** Decía «—doce cerradas
                             o registradas—» (hoy no son doce: `C-L.5` está ABIERTA y
                             `C-L.7` NO CERRADA), decía que `C-L.5` «SIGUE CERTIFICADA»
                             (el CUARTO GATE la reabrió) y mandaba buscar la clasificación
                             vigente «más abajo», cuando está ARRIBA: quien obedeciera el
                             puntero llegaba a la versión anterior al documento 22. **La
                             corrección no sustituye un estado por otro: retira los dos
                             ...
                             **Y VOLVIÓ A CADUCAR, hasta la tanda posterior al documento 26:
                             la lista se detenía en la 10ª mientras el árbol iba por el
                             documento 26, TRES gates más allá.** Por eso las pasadas nuevas
...
>
> FILAS ADVERSARIALES  **[HISTÓRICO · `EE-12` del SEXTO GATE. El renglón decía «46 filas
>                      físicas · 46 ids únicos en §2.6.7», dentro de un bloque rotulado
>                      CIFRAS VIGENTES, DERIVADAS, y hoy son más: `X63` la añadió la tanda
>                      del quinto gate. Era cierto el día en que se escribió, que es
>                      exactamente lo que hace que caduque sin que nadie lo note. NO se
>                      sustituye por el vigente: se retira y se remite.]**
>                      La cifra vigente vive en §2.6.7 y se deriva con
>                      `grep -cE '^\| `?X[0-9]' docs/evolucion/11-ARQUITECTURA-INTEGRADA.md`
> ```
>
...
### `DD-20` · LA FRONTERA ENTRE `A` Y `C`, FIJADA AQUÍ, QUE ES SU SEDE

> **Fijada por `DD-20` del QUINTO GATE, y hasta hoy el corpus llevaba DOS enunciados
> vigentes e INCOMPATIBLES.** De cuál rigiera dependía si un árbol defectuoso **cuenta**
> como hallazgo o se descarta como ataque privilegiado — es decir, dependía el veredicto.
>
> ```text
> FORMULACIÓN 1 · `AA`, documento 25 §8   «`A` = … sin tocar la batería, su README, `HEAD`,
>                                          las refs, la base ni el runner, **y SIN COMMITEAR**»
> FORMULACIÓN 2 · `Z3`, documento 25 §4   «`A` = el defecto está EN EL CORPUS y la batería
>                                          dice N/N. Que esté commiteado es IRRELEVANTE: el
>                                          corpus que un gate juzga es un COMMIT»
> ```
>
> **RIGE LA FORMULACIÓN 2**, y lo resolvió `DD` contra las dos sedes, no por mayoría: ni
> este bloque ni `O18` mencionan commitear, y los seis actos que `O18` enumera como NO
> protegidos son compromiso de canal, de repositorio y coordinador a la vez, robo de
> credenciales, reescritura autorizada de ramas remotas, manipulación del ejecutor externo
> y falsificación de identidad. **Hacer un commit ordinario en la rama que se somete a
> revisión no es ninguno de los seis: es el acto ordinario del coordinador.** Y lo confirma
> el propio `AA`, que clasificó su árbol COMMITEADO `AA-E4b` como `A`+`B` pese a escribir
> «sin commitear» en su enunciado.

```text
`A` COHERENCIA INTERNA    el defecto está EN EL CORPUS y la batería lo da por bueno. Que el
                          fichero esté o no CONFIRMADO es IRRELEVANTE: el objeto que un gate
                          juzga es un COMMIT, y confirmar es lo que hace el coordinador en su
                          propia rama de revisión

`C` ACTOR PRIVILEGIADO    corromper la REFERENCIA contra la que se compara: reescribir `HEAD`,
                          las refs o la revisión base · editar la batería, su README o el
                          derivador · mentir el runner · cualquiera de los SEIS actos que
                          `O18` enumera como NO protegidos. Es contrato de `F6`, y NO es
                          exigible dentro de `F4c`

CONSECUENCIA, Y ES LA     un árbol defectuoso **COMMITEADO** que la batería no ve es clase
QUE DECIDE                `A`, cuenta, y no se descarta por estar confirmado
```

### `DD-19` · DE QUÉ ÁRBOL HABLA CADA CIFRA DE UN MANIFIESTO — regla para todo gate posterior

> **Fijada por `DD-19` del QUINTO GATE, y no es un descuido de una tanda: es un PUNTO
> FIJO.** Desde que `AA-01` convirtió `manifiestos/` en ZONA BARRIDA, el manifiesto del gate
> en curso **entra en su propio universo**, y §5.4 obliga a que cada fila publique el
> SHA-256 de su fuente. **Un manifiesto no puede contener su propio SHA-256**: fijarlo lo
> cambia. Por tanto `OBLIGATORIO − ASIGNADO = 0` **sobre el árbol del GATE es INALCANZABLE,
> y lo será en todos los gates siguientes**. No es un hallazgo contra una tanda: es una regla
> del corpus que ninguna tanda puede satisfacer.

```text
LA REGLA                  todo manifiesto ROTULA, cifra a cifra, DE QUÉ ÁRBOL habla, y
                          PUBLICA LAS DOS ARITMÉTICAS: la del árbol CANDIDATO —que es el
                          objeto del reparto y donde la resta SÍ cierra a cero— y la del
                          árbol del GATE, donde la diferencia es exactamente el manifiesto
                          en curso y se dice así

POR QUÉ ESTA VÍA Y NO     `DD` ofreció tres, las tres de `F4`. Se toma la primera —«cuesta
    OTRA                  una palabra»— junto con la segunda. **NO se toma la tercera**
                          —que el derivador excluya de su barrido el manifiesto en curso—:
                          un instrumento que se saca a sí mismo del universo que deriva es
                          exactamente la clase de perímetro escrito que `DD-01` cierra un
                          piso más abajo, y cerrarla arriba para reabrirla aquí sería el
                          modo de fallo que este expediente lleva cinco gates persiguiendo

LO YA PUBLICADO           el manifiesto del QUINTO GATE es INMUTABLE y NO se edita: su
                          rótulo queda acotado en las entradas §14 y §15 del
...
FALLÓ EN SU PRIMER USO    `OBLIGATORIO − ASIGNADO = 1` sobre el árbol del gate y son **2**:
                          razonó bien la exención del manifiesto EN CURSO —que es punto
                          fijo— y la extendió sin decirlo al manifiesto que sustituía, que
                          NO lo es. Desde el gate siguiente:
                            · el §6 **DERIVA las dos aritméticas con su comando** en vez de
                              copiarlas. Un cardinal escrito en un manifiesto es un
                              cardinal que nadie vuelve a mirar
                            · **toda fuente que quede sin fila lleva su razón derivada y
                              publicada, UNA A UNA**. La exención de punto fijo cubre al
                              manifiesto EN CURSO y a **ningún otro fichero**
                            · y el preámbulo del §5 **describe la regla que se ejecuta** —
                              bytes idénticos al árbol que ESE gate leyó— y no una más
                              estricta que sus propias filas no pueden satisfacer (`EE-08`)
```

## PARTE DE LA TANDA POSTERIOR AL QUINTO GATE — un renglón por hallazgo, y el recuento se DERIVA

> **Por qué existe esta sección, y por qué NO lleva ningún cardinal.** Un parte que dice
> «se han aplicado N hallazgos» obliga a quien lo lee a fiarse de una suma que nadie vuelve
> a hacer, y `DD-13` demostró que en este corpus esas sumas caducan. **Aquí no hay ninguna
> suma escrita: hay una fila por identificador**, y quien quiera el número lo saca del
> árbol con el comando de abajo. Es la regla de titulares de §0 del documento 11 aplicada
> al registro que la aplica.
>
> **CÓMO SE CUENTA, y es la única sede del recuento:**
>
> ```bash
> # `EE-10` · cuenta IDENTIFICADORES DISTINTOS, no filas: dos filas con el mismo id serían
> # un doble conteo, y contar filas no lo vería. Y acota la selección a ESTA tabla.
> awk '/^### Lo aplicado, un renglón por identificador/{t=1} t' \
>   docs/evolucion/CHECKPOINT-ADS-NEXT.md |
>   grep -oE '^\| `(DD|BT)-[0-9]+`' | sort -u | wc -l
>
> # y el desglose, por el mismo camino
> awk '/^### Lo aplicado, un renglón por identificador/{t=1} t' \
>   docs/evolucion/CHECKPOINT-ADS-NEXT.md |
>   grep -oE '^\| `(DD|BT)-[0-9]+`' | sort -u | cut -c4-5 | sort | uniq -c
> ```
>
> > **PRECISADO por `EE-10` del SEXTO GATE.** El comando anterior contaba **filas de
> > cualquier tabla del fichero** con `awk '/^\| `(DD|BT)-[0-9]/{n++}'`, y la frase lo
> > presentaba como «el recuento». Dos defectos: **contaba filas y no identificadores
> > distintos** —un id repetido en dos filas se habría contado dos veces— y **no acotaba la
> > tabla**, de modo que cualquier renglón futuro con esa forma en otra sección habría
> > entrado en el censo. La CONCLUSIÓN no cambia y se conserva: **el cardinal se retira y se
> > publica la enumeración**. Lo que se corrige es que el comando mida lo que la frase dice
> > que mide, que es la exigencia de `J-07` aplicada al propio instrumento de la
> > reconciliación.
>
> **APLICAR NO ES CERTIFICAR.** Ningún hallazgo de esta tabla se declara SUPERADO: eso lo
> hace un gate independiente y nadie más. Lo que la tabla afirma es que el remedio está
> ESCRITO y, donde había control posible, EJECUTADO con su salida.

**LA RECONCILIACIÓN DE «SIETE HALLAZGOS APLICADOS», hecha mecánicamente y no por lectura.**
La primera pasada de esta tanda —la que tocó **sólo** `11-ARQUITECTURA-INTEGRADA.md`— se
resumió como «siete hallazgos aplicados», y sus identificadores son **seis del gate más uno
del barrido**:

```text
LOS SEIS DEL GATE, verificables en el propio documento 11:
    DD-05  DD-07  DD-09  DD-10  DD-13  DD-14
      grep -c 'DD-0[579]\|DD-1[034]' docs/evolucion/11-ARQUITECTURA-INTEGRADA.md

EL «DEFECTO ADICIONAL DESCUBIERTO DURANTE EL BARRIDO»
    el barrido transversal no encontró UNO: encontró DOS, y los dos están medidos y
    corregidos en esta misma tabla — `BT-01` y `BT-02`

POR TANTO
    6 + 1 = 7  es la aritmética que la frase supone
    6 + 2 = 8  es la que el árbol sostiene
    y ninguna de las dos describe la tanda COMPLETA, que aplica los del gate ENTERO
```

**Consecuencia, y es la razón de que la frase se retire en vez de corregirse:** «siete» era
cierto de una PASADA y se estaba leyendo como cierto de la TANDA; y dejó de ser cierto
incluso de la pasada en cuanto el barrido devolvió su segundo hallazgo. **Un cardinal cuya
verdad depende de qué recorte se mire es exactamente el defecto que `DD-13` castiga.** No se
sustituye un número por otro: **se retira el número y se publica la enumeración con su
comando**, que es lo que el corpus exige desde `J-07` y lo que esta tanda hace en todas las
demás sedes que tocó.

### Lo aplicado, un renglón por identificador

| id | sede del remedio | qué se hizo, y con qué prueba |
|---|---|---|
| `DD-01` | `derivar-universo-obligatorio.py` · `comprobar-correccion-gate-de-cierre.py` · README de `verificacion/` | **EL OCTAVO ÁRBOL.** El perímetro pasa a excluir por lo que un fichero **ES**: `.git` **anclado a la RAÍZ** de la ruta relativa, con la poda de `os.walk` evaluada **sobre la RUTA COMPLETA** en todas sus sedes de los dos instrumentos, y el bytecode **por su CONTENIDO** —cabecera de CPython y no-texto—, nunca por su sufijo. **Y todo lo excluido se PUBLICA con su ruta y su motivo** (`EXCLUIDOS_PERIMETRO`), por todos los modos del derivador y en el detalle de `G-29`. **CONTROL POSITIVO EJECUTADO:** el mismo documento del adjudicador —`docs/owner/sentencia.pyc` declarando `F4c` cerrada y `F5` autorizada— **entra hoy en el universo y pone `G-29` en ROJO**; con el instrumento anterior no entraba y daba verde |
| `DD-02` | `comprobar-correccion-gate-de-cierre.py` · README | **La admisión de `docs/owner/` se evalúa sobre el CONTENIDO DEL COMMIT**, contra `git ls-tree -r HEAD docs/owner/`, y no sólo sobre lo que aún no está en `HEAD`. La guarda era INERTE para todo fichero ya confirmado: `37/38` sin commitear y **`38/38` commiteado**, sobre un objeto que es un COMMIT. **CONTROL POSITIVO EJECUTADO:** una sede de `docs/owner/` ya en `HEAD` a la que se le quita el enlace del índice da hoy ROJO, nombrándola |
| `DD-03` | README de `verificacion/`, fila `G-29` | La fila **dice lo que el código hace**: perímetro por RAÍZ y por CONTENIDO, con lo excluido publicado. Decía «por su extensión» bajo el titular «por NATURALEZA», y las dos mitades eran falsas |
| `DD-04` | `comprobar-correccion-gate-de-cierre.py` · README, fila `G-23` | **La zona normativa se DERIVA del árbol —`docs/rediseno/` entera— y lo que se enumera es su EXCEPCIÓN, con motivo**, igual que ya se hacía con el kernel. El regex de seis patrones cubría cuatro de sus catorce ficheros. **CONTROL POSITIVO EJECUTADO:** el `00-MAPA.md` del adjudicador —declarando que prevalece sobre el material APROBADO y deja `O17`·`O18`·`O19` sin efecto— da hoy ROJO; antes daba `38/38` |
| `DD-05` | documento 11, §11.9 | **Reincidencia LITERAL de `Y-09`.** «`CI` o el ejecutor externo» sale del bloque rotulado LITERAL, que vuelve a ser la sede cláusula a cláusula; las dos glosas que la sede no tiene bajan a un bloque propio rotulado **LECTURA DEL COORDINADOR — NO ES LITERAL DE LA SEDE**, y el literal se deriva con su `sed` |
| `DD-06` | `DECISIONES-Y-CONTRADICCIONES.md` · `00-INDICE.md` · este fichero | **Reincidencia de `Y-05`, que sobrevivió porque se barrieron las COMILLAS y no el ACTO.** «Robustez y revalidación permanente por encima del ahorro operativo» **no es del Owner** —`grep -c robustez` sobre la sede canónica da `0`— y pasa a estar rotulada como RESUMEN del registro que la escribe, en **todas** sus sedes vivas, con el literal separado y con la sede a la que remite. Lo que queda dentro de una región HISTÓRICA se conserva sin tocar, y se dice |
| `DD-07` | documento 11, `C-L.5` §11 (dos sedes) | **`C-L.5` tenía DOS estados vigentes, y el que discrepaba vivía en la sede que el propio documento designa como suya.** Las dos sedes **retiran el estado en vez de sustituirlo** —`P-22`≡`Q-37` ya lo había sustituido una vez y volvió a caducar— y remiten a la clasificación vigente del checkpoint, con su `grep` |
| `DD-08` | este fichero, bloque «Estado de las fases» | **Tres defectos en un renglón que declaraba no copiar:** «doce cerradas o registradas» (falso), «`C-L.5` SIGUE CERTIFICADA» (falso) y un puntero que mandaba «más abajo» cuando la clasificación vigente está ARRIBA. **Se retiran los dos estados y se endereza el puntero**, con el comando que lo localiza. Sexta recurrencia de la clase que `C-L.7` existe para cerrar |
| `DD-09` | documento 11, tres sedes normativas + barrido de clase | **`D105` propagado**: `abandonada` **NO retira el marcador al emitirse**. Se corrigen el discriminador, la tabla de cardinalidad y la sede del ORDEN, y **se barre la CLASE** —toda sede que afirme qué retira el marcador— en vez de las ocho que `Y-02` enumeró. El barrido de clase se reejecutó en esta tanda y no quedan sedes vivas con la forma previa |
| `DD-10` | documento 11, §2.1 | **`X-S1`–`X-S9` era falso: son más.** El censo **deja de escribirse** y remite a su sede, §9.6, con el comando que lo deriva. Es la sede definitoria, y la que gobierna la prueba de `D83` contratada para `F6` |
| `DD-11` | `comprobar-correccion-gate-de-cierre.py` · README, fila `G-33` | **La única prueba negativa ANCLADA EN EL ÁRBOL era TAUTOLÓGICA** —el mutante borraba **las dos** cadenas gatillo, y el adjudicador midió 200 000 textos aleatorios con **cero** contraejemplos—. Hoy **se muta UNA sola cadena y se exige que la denuncia sea EXACTAMENTE la suya**. **CONTROL EJECUTADO:** se construyó un corpus alternativo, igual de válido, que **rompe la aserción** — con eso deja de ser tautológica y la promesa «un corpus distinto la mueve» pasa a ser cierta |
| `DD-12` | `comprobar-correccion-gate-de-cierre.py` · README · documento 11 §0 | **`G-10` no DERIVABA nada**, y era el aval con el que §0 se permite escribir un cardinal. Hoy **deriva el censo de la enumeración `capacidades/<CAP>/` de §5.2** y de ahí saca el cardinal en letra, el cardinal en cifra y los nombres que §5.2, §16 y §17 deben publicar. **CONTROL POSITIVO EJECUTADO:** una SÉPTIMA extensión de ficha pone hoy en ROJO **las tres sedes a la vez**, nombrándola; antes pasaba en verde. La viñeta de §0 que ampara la excepción queda precisada: **sólo vale mientras el guardián derive** |
| `DD-13` | documento 11, seis sedes + `X63` + barrido de clase | **La regla de titulares sin guardián, y sus instancias vivas.** Los titulares numéricos que su propia enumeración desmentía pasan a REMITIR o a DERIVARSE; §5.6 dejaba su tesis central **sin contrato de prueba** atribuyéndola a `X52`, una fila existente pero AJENA —reincidencia literal de `J-03`—, y se contrata la fila que faltaba, **`X63`**. El barrido pasa a ser sobre `^\*\*.*<cardinal>` y no sólo sobre `^#`, que es la observación de método de `BB1`; **reejecutado en esta tanda sobre las siete sedes vivas del corpus, sin defecto residual** |
| `DD-14` | documento 11 §11.7 · `DECISIONES-Y-CONTRADICCIONES.md`, entrada `O17` | **Asimetría de procedencia, y se corrige el PAR ENTERO.** El rótulo «literal de `O18`» que `O19` ordena reatribuir se había corregido en §11.8 y no en §11.7, a 142 líneas; y la proyección de `O17` **no nombraba ni una vez** la sede que la designa como proyección —`grep -c owner` daba `0`—, mientras la de `O19` sí. Los dos lados corregidos, con el censo de proyecciones enlazadas **derivado por `awk`** |
| `DD-15` | README de `verificacion/` · docstring de `_regiones_historicas` | **Tercera vida de `Z1-06`:** la nota decía «se retira el que había caducado, y queda uno solo» y **quedaban los dos**. Hoy **no queda ninguno**: el estado de la sección lo dice la TABLA, y se publica el `git status` que lo contrasta. Y la enumeración de fronteras de cierre de región **gana la excepción que el código sí aplica** a la clase `tanda`, que cierra en columna 0 y no en línea en blanco |
| `DD-16` | `00-INDICE.md` | El punto de entrada decía que la siguiente acción está «**al final**» del checkpoint y está **la PRIMERA de varias**; quien obedecía el puntero llegaba a la más antigua. Hoy dice lo mismo que la cabecera del checkpoint ya decía, y **el número de versiones históricas no se escribe: se deriva** |
| `DD-17` | este fichero, «Siguiente acción exacta» punto 6 | **QUINTA recurrencia consecutiva**: el commit del gate deja el árbol que juzga con un validador canónico en ROJO por commitear el manifiesto **solo**. La regla queda escrita donde el coordinador la lee antes de encargar: **el commit del manifiesto lleva el manifiesto, SU FILA EN `00-INDICE.md` y la evidencia derivada reejecutada**. Es una regla de EJECUCIÓN, y se ejecuta en el commit del gate siguiente; esta tanda la registra y no puede cerrarla por él |
| `DD-18` | `CORRIGENDUM-DICTAMENES-INMUTABLES.md` §14 | El manifiesto del quinto gate **rotula «sobre el árbol del GATE» cifras que son de la CANDIDATA**. Un manifiesto es INMUTABLE: la vía es el corrigendum, y su entrada publica **las dos derivaciones con su comando** —candidata y árbol del gate—, y nombra la fuente que las separa: el propio manifiesto en curso. Tercera instancia de una clase declarada ROTA un gate antes |
| `DD-19` | `CORRIGENDUM` §15 · este fichero, «El criterio del gate siguiente» | **`OBLIGATORIO − ASIGNADO = 0` sobre el árbol del gate es INALCANZABLE por PUNTO FIJO**, y lo será siempre: el manifiesto tendría que contener su propio SHA-256. Se toman **la primera y la segunda** de las tres vías de `DD` —rotular de qué árbol habla cada cifra **y** publicar las dos aritméticas— y **se rechaza expresamente la tercera**, que sacaría al derivador del universo que deriva: eso reabriría un piso más arriba la puerta que `DD-01` acaba de cerrar |
| `DD-20` | este fichero, «El criterio del gate siguiente» | **El corpus llevaba DOS enunciados vigentes e incompatibles de la frontera `A`/`C`**, y de cuál rigiera dependía si un árbol defectuoso cuenta. Queda fijado en su sede con la formulación de `Z3`, que es la que sostienen la sede y `O18`: **el objeto que un gate juzga es un COMMIT, y confirmar es el acto ordinario del coordinador, no un privilegio** |
| `DD-21` | `comprobar-correccion-gate-de-cierre.py` · README, sección propia | **Ninguna sede acotaba lo que un «N/N en verde» certifica.** Hoy la batería **DERIVA de los títulos** de sus propias comprobaciones cuáles exigen un repositorio con historia y lo publica en su informe, y el README dice qué significa su ausencia. Y `G-34` **gana en su título el «falla CERRADO sin git» que le faltaba**: era la que hacía que lo derivado y lo medido no cuadraran. **MEDICIÓN REPRODUCIDA:** sobre la materialización sin `.git` que prescribe la receta del sobre, la lista derivada y la lista de las que caen **coinciden exactamente** |
| `DD-22` | `CORRIGENDUM`, regla general | **Ninguna huella se abrevia a mano en ningún informe.** Si se abrevia, se DERIVA. Es la clase exacta que invalidó el cuarto gate, reaparecida en la columna abreviada de una fila de relevo |
| `BT-01` | `DECISIONES-Y-CONTRADICCIONES.md`, nota de trazabilidad de `O17` | **Hallazgo del BARRIDO TRANSVERSAL de esta tanda; no lo señaló ninguno de los ocho participantes del quinto gate.** La nota declaraba el motivo de `O17` **INVERIFICABLE** porque «no hay otra sede», y **publicaba al lado el comando que la desmiente**: hoy devuelve más de una línea, y una de ellas es la SEDE CANÓNICA que `O19` creó. **Una afirmación caducada dentro de una nota que llevaba su propio refutador escrito a un renglón**, superviviente de dos gates. Se corrige la parte que la sede cierra —dónde manda el texto— y **se conserva declarada la que sigue abierta**: la trazabilidad de la RESPUESTA, que sigue sin fuente primaria |
| `BT-02` | `comprobar-correccion-gate-de-cierre.py`, mensaje de `G-16` | **Hallazgo del BARRIDO TRANSVERSAL de esta tanda.** El mensaje de éxito de `G-16` terminaba en el literal **«C-L.5 CERTIFICADA»** y `C-L.5` está **ABIERTA** desde el cuarto gate: **la batería imprimía en verde una afirmación FALSA sobre la condición que el gate acababa de reabrir, dentro de la comprobación cuyo objeto es que nadie copie estados**, y a nueve líneas del comentario que declara haber retirado por ese mismo motivo la guarda escrita a mano. **Instancia corregida, clase abierta** — la frase de `BB4`, medida en el propio instrumento. Hoy el estado se DERIVA de la sede canónica y el informe lo dice |

### Lo que esta tanda NO ha hecho, y es deliberado

```text
NINGUNA COMPROBACIÓN NUEVA   el censo de la batería no se mueve. No se escribe: lo publica
                             ella, y `G-34` lo contrasta contra su README en las dos
                             direcciones

NINGÚN HALLAZGO SUPERADO     ni uno. `M-04` sigue abierta, `C-L.5` sigue ABIERTA y `C-L.7`
                             sigue NO CERRADA. **APLICAR NO ES CERTIFICAR**

NADA INMUTABLE EDITADO       ni la SEDE CANÓNICA del Owner —es append-only—, ni el texto
                             resolutivo de ninguna `O` ni de ninguna `D`, ni ningún dictamen
                             de gate, ni ningún manifiesto. Lo de `DD-18` y `DD-19` va al
                             CORRIGENDUM, que es la vía que la regla §16 de ese documento
                             prescribe

NINGÚN NÚMERO SUSTITUIDO     donde se podía RETIRAR y REMITIR, se retira y se remite.
POR OTRO                     Sustituir un cardinal caducado por el vigente es aplazar el
                             defecto un gate, y este expediente ya lo hizo tres veces

`X63` NO ES UNA PRUEBA       es **CONTRATO DE PRUEBA DE `F6`**, como las demás filas de
                             §2.6.7. NO se ha ejecutado, NO certifica nada y NO es una
                             protección interna nueva. Escribir el contrato de una prueba
                             no es la prueba

NADA DE GIT MÁS ALLÁ DE      no se ha hecho merge, no se ha movido `redesign/kernel-2.0` y
LA RAMA DE TRABAJO           no se ha reescrito ninguna referencia
```

## PARTE DE LA TANDA POSTERIOR AL SEXTO GATE — un renglón por hallazgo, y el recuento se DERIVA

> **Misma disciplina que el parte anterior, y por la misma razón:** aquí no hay ninguna suma
> escrita. Hay **una fila por identificador**, y quien quiera el número lo saca del árbol.
>
> ```bash
> # los IDENTIFICADORES que la tanda aplica — incluidos los que comparten fila
> awk '/^### Lo aplicado por la tanda del SEXTO GATE/{t=1} t' \
>   docs/evolucion/CHECKPOINT-ADS-NEXT.md |
>   grep -oE '`EE-[0-9]+`' | sort -u | wc -l
>
> # y la cobertura contra el gate, que es lo que hay que comprobar: sale VACÍO
> comm -23 <(grep -oE '^\| [0-9]+ \| \*\*`EE-[0-9]+`' \
...
> ```
>
> **Hay MENOS filas que identificadores, y es deliberado:** `EE-07` y `EE-12` son el mismo
> defecto en dos sedes y comparten fila, como el adjudicador los agrupó en su remedio. Por
> eso el comando cuenta IDENTIFICADORES y no filas — `EE-10`, aplicado a este mismo parte.
>
> **APLICAR NO ES CERTIFICAR.** Ningún hallazgo de esta tabla se declara SUPERADO.

**LO QUE ESTA TANDA ES, EN UNA LÍNEA.** El adjudicador `EE` dictaminó que el defecto **no
está en la ejecución de la tanda anterior sino en el ALCANCE de un remedio**: `DD-02` cerró
la guarda de admisión **para una zona**, y la clase quedó abierta. Esta tanda **deriva el
alcance** en vez de ampliarlo zona a zona, que es la única forma de que la corrección no
tenga que repetirse en el gate siguiente.

### Lo aplicado por la tanda del SEXTO GATE

| id | sede del remedio | qué se hizo, y con qué prueba |
|---|---|---|
| `EE-01` | batería · README de `verificacion/` | **EL NOVENO ÁRBOL, cerrado por CLASE.** La guarda de admisión **se evalúa contra la REVISIÓN BASE y no contra `HEAD`**: es AMPLIACIÓN todo lo que existe hoy y no existía en la base, **esté commiteado o no**. Confirmar deja de ser una forma de admitirse. Y las zonas que no tenían condición la reciben: los manifiestos, **enlazados desde la LISTA de `00-INDICE.md`**, que es lo que `C-L.5` ya exigía; y el ordinal de un documento numerado deja de exigirse LIBRE y pasa a **prohibir la COLISIÓN**, que es lo que la regla quería decir. **CONTROL POSITIVO EJECUTADO, y con las CINCO variantes COMMITEADAS** —`docs/normativa/`, la RAÍZ, `docs/`, un documento numerado con ordinal libre sin enlace, y un manifiesto sin enlace—: **las cinco dan ROJO hoy**, y las cinco daban `38/38` antes. Y el título de `G-29` y la fila del README **dicen lo que el código hace**, que es la sexta condición de `O18` |
| `EE-02` | `CORRIGENDUM` §16 · «El criterio del gate siguiente» | El manifiesto `6B` publica `OBLIGATORIO − ASIGNADO = 1` sobre el árbol del gate y son **2**: razonó bien la exención de punto fijo del manifiesto EN CURSO y **la extendió sin decirlo al que sustituía**, que no lo es — 278 líneas obligatorias sin asignar. Un manifiesto es INMUTABLE: la vía es el corrigendum, con las dos cifras derivadas y su comando. **Y la regla de `DD-19` se endurece donde falló**: el §6 DERIVA las dos aritméticas y **toda fuente sin fila lleva su razón, una a una** |
| `EE-03` | `00-INDICE.md` | **CUARTA recurrencia de `S-18`≡`T-14`.** Los manifiestos del quinto y del sexto gate estaban enlazados **desde su fila y no desde la LISTA**, que es lo que la regla exige. Se añaden los tres. **Y el comando que la sede publica para autocomprobarse se ACOTA A LA LISTA**: barría el índice ENTERO, de modo que un enlace en una fila histórica ya lo satisfacía y **el instrumento que existe para detectarlo pasaba en verde**. Verificado: el `diff` sale vacío |
| `EE-04` | `CHECKPOINT`, bloque de estado | **QUINTA recurrencia de la clase `C-L.7`, y la PRIMERA contra la regla escrita DENTRO del propio bloque para impedirla.** `metodo`, `last_meaningful_event` y `based_on` nombraban el CUARTO gate con el árbol en el SEXTO. Los tres **REANCLADOS**, y lo anterior baja a sus campos `_anterior` como la regla 5 ordena. **Reanclar no es certificar**: `C-L.7` sigue NO CERRADA |
| `EE-05` | `CHECKPOINT`, filas `C-L.5` y `C-L.7` | **`AA-02` VIVO en la fila gemela de la que lo declaró cerrado.** `C-L.5` conservaba «*mientras el primero devuelva 0, `G-16` dará ROJO*» con el antecedente **falso desde `8c9ca9c`**, y `C-L.7` **remitía a esa aserción** como si el rojo siguiera vivo: el defecto se sostenía entre las dos. **Se retiran las dos y se publica el comando que lo decide** |
| `EE-06` | `11-ARQ` §0 | **La sede se falsaba con el comando que ella misma publica**, y lo rompió **el remedio de `DD-12`, en el mismo commit y doce líneas más abajo**. Se retira la afirmación caducada y se dice **lo que hay y lo que no**: `G-10` deriva UNA instancia y la guarda; **no existe comprobación GENERAL de la regla**, que sigue siendo la deuda que `DD-13` declaró |
| `EE-07` · `EE-12` | `CHECKPOINT`, dos sedes | Los dos cardinales `46` de la tabla adversarial —uno en un renglón vivo, otro dentro de un bloque rotulado «CIFRAS VIGENTES, DERIVADAS»— **se RETIRAN y se REMITEN**, nunca se sustituyen por el vigente. Es el cardinal que `DD-13` retiró del documento 11 **en la tanda anterior**, superviviente dos sedes más allá |
| `EE-08` | `CORRIGENDUM` §17 · «El criterio del gate siguiente» | El preámbulo del §5 del manifiesto describía una regla **más estricta** que la que aplica, y **dos de sus sesenta filas no pueden satisfacerla** porque el gate que las certificó no publicó SHA-256 para ellas. El agotamiento **no queda en duda** —el adjudicador verificó las 76 filas sin discrepancia—: lo que falla es la descripción |
| `EE-09` | derivador, `_leer` | **Falla CERRADO también ante una sede ilegible por CODIFICACIÓN.** El `except` sólo capturaba `OSError`; un fichero que abre y **no decodifica** mataba el proceso con traza y `rc=1` **sin el diagnóstico `FALLA CERRADO`**, y la receta del sobre —que canaliza con `2>/dev/null`— entregaba **una lista VACÍA en silencio**. Misma clase que `T-22`, en el fichero de al lado |
| `EE-10` | `CHECKPOINT`, la reconciliación | El comando **contaba filas de cualquier tabla del fichero**, no identificadores distintos de ESTA. Hoy cuenta **identificadores únicos** y **acota la tabla**. La conclusión —retirar el cardinal— **se conserva**: lo que se corrige es que el comando mida lo que la frase dice medir |
| `EE-11` | batería | **La salida de git deja de partirse por blancos.** `.split()` sobre `ls-tree`/`diff` fallaba de dos maneras a la vez: una ruta **con espacio** se troceaba y producía el diagnóstico FALSO «fichero DESAPARECIDO», y una ruta **no-ASCII** salía citada por `core.quotePath` y **el bucle de admisión no la veía**. Con `-z` desaparecen las dos |
| `EE-13` | `DECISIONES`, glosa de `D103` | «La batería sigue teniendo **30 comprobaciones**», en PRESENTE y dentro de un registro VIVO. Se retira el cardinal, **se conserva lo que sigue siendo cierto** —que `G-15` se corrigió en su sitio, sin añadir ninguna— y se publica el comando |
| `EE-14` | `CORRIGENDUM` §18 | El §5 del documento 26 publica un desglose por origen que sus filas no dan. **El documento es INMUTABLE y no se toca**: se acota en el corrigendum, que es la vía que el propio corpus prescribe y que aquí no se había usado. **El TOTAL y la CLASIFICACIÓN son correctos y se dice** |
| `EE-15` | emisor | Se retira la afirmación **falsa y medida** de que `read-tree`+`checkout-index` «no consultan `.gitattributes`» —sí leen los de conversión; lo que no honran es `export-ignore`, que es lo único que el remedio necesitaba— y **queda declarada la divergencia estructural** entre lo que el derivador VE y lo que el digest MIDE |
| `EE-16` | emisor | El recuento de líneas **deriva de una sola sede**: el emisor escribía su propia fórmula y divergía de la del derivador en el único caso en que difieren, el fichero vacío |
| `EE-17` | batería | **El ALCANCE deja de derivarse de una convención de redacción.** Se declara la PROPIEDAD —qué comprobaciones contrastan contra la historia— en un solo sitio, el ALCANCE sale de ahí, y **el TÍTULO se contrasta contra ella**: si divergen en un identificador, se dice. Es la convención de la que nació `DD-21` |
| `EE-18` | `11-ARQ` §11.9 | El rótulo de la **sexta** cláusula del bloque LITERAL era una CARACTERIZACIÓN del coordinador y no el SUJETO, como los cinco anteriores. `DD-05` limpió cinco y **no barrió la sexta** |
| `EE-19` | emisor, obligación 4 | El sobre prometía la «**superficie EXACTA** en que candidata y gate no son el mismo objeto» sobre un **subconjunto propio**: las rutas del UNIVERSO, no las del ÁRBOL. Se dice lo que la lista es **y se publica el comando que da la otra** |

### Lo que esta tanda NO ha hecho, y es deliberado

```text
NINGUNA COMPROBACIÓN NUEVA   el censo de la batería no se mueve. Lo publica ella, y `G-34`
                             lo contrasta contra su README en las dos direcciones. Lo que
                             `EE-01` pidió era de RESTA y de ALCANCE, y así se hizo

NINGÚN HALLAZGO SUPERADO     ni uno. `M-04` sigue abierta, `C-L.5` sigue ABIERTA y `C-L.7`
                             sigue NO CERRADA. **APLICAR NO ES CERTIFICAR**

NADA INMUTABLE EDITADO       ni la SEDE CANÓNICA del Owner, ni el texto resolutivo de
                             ninguna `O` ni de ninguna `D`, ni ningún dictamen de gate, ni
                             ningún manifiesto —el `6` sustituido incluido—. `EE-02`,
                             `EE-08` y `EE-14` van al CORRIGENDUM
...
NADA VUELVE AL OWNER         `EE` examinó cuatro candidatos a clase `B` y los cuatro caen.
...
| `C-11` | CORREGIDO_EN_F4c | sí | sí | no | no | no | `PLT` | `F4c` la afirmación · `F6` la unificación | el derivador no dice «ÚNICA SEDE» donde hay tres usos; la unificación va a **`V6-19`**, que es el contrato que CONTIENE materialmente esa obligación | derivador · emisor · §20.1 `V6-19` |
...
| `C-20` | CORREGIDO_EN_F4c | sí | no | no | no | no | `PLT` | `F4c` | el SOBRE transporta el TEXTO ÍNTEGRO de la ratificación `O19`, no sólo sus digest, y el receptor puede comprobar con `sha256sum` que lo entregado es lo anclado | `O19` L315-317 · §11.6 campos 17-18-19 y paso `6ter` · emisor |
...
LOS QUE PERMITEN UN           `C-00` · `C-01` · `C-02` · `C-03` · `C-04` · `C-13`, y `C-18`
FALSO VERDE                   con ellos por ser superficie del mismo verificador. Bloquean
                              `F6` y bloquean PesquerApp, y así consta en su fila. **El
                              cardinal se retira de este rótulo** —`H-14`, regla de `J-07`—:
                              lo deriva la tabla, no esta frase
                                awk '/^\| `C-[0-9]+` \|/' CHECKPOINT-ADS-NEXT.md |
                                  grep -c '| \*\*SÍ\*\* | \*\*SÍ\*\* |'

`C-20` SALE DE ESE GRUPO      `H-01`. Estaba con fase `F6` y prueba de cierre `V6-16`·`V6-17`,
                              y **las dos eran falsas**: su sede normativa —`O19` L315-317 y
                              §11.6— le da fase `F4c` y propietario `PLT`, y ninguno de los
                              dos contratos menciona el CONTENIDO del sobre, de modo que
                              `F6` podía cerrarlos los dos en verde con el defecto intacto.
                              Su estado primario pasa a **CORREGIDO_EN_F4c** y su remedio se
                              aplica en el emisor, no se contrata.
                              **La rama alternativa —que §11.6 y la sede DEJARAN de exigir el
                              texto— NO estaba disponible sin el Owner**: esa lista vive
                              dentro de una sede APPEND-ONLY, y retirarla es su decisión
...
`C-11` LLEVA LAS DOS COSAS    su ESTADO PRIMARIO es CORREGIDO_EN_F4c —la afirmación falsa
                              ...
                              fórmula en las tres sedes es trabajo de `F6`.
                              **Un estado primario, un atributo secundario, y no se mezclan**

`C-11` YA NO CUELGA DE        `H-05`. Su mitad `F6` iba a `V6-04`, que contrata **otra
`V6-04`                       propiedad**: un censo DERIVADO de las lecturas de Git. Una
                              obligación enrutada a un contrato que no la contiene se cierra
                              en verde con la obligación sin cumplir, que es la misma forma
                              de `H-01`. Va a **`V6-19`**, escrito con todos los campos que
                              §20.1 define en su cabecera, cuyo
                              objeto es exactamente el suyo: **una sola sede por fórmula
                              compartida, y consumidores que la IMPORTAN**. Ni se mezclan
                              propiedades ni se declara implementación: `V6-19` es contrato
```

### `CONTRATO_COMPLETO_PARA_F6` NO ES «CONSTRUIBLE», y desde ahora la tabla lo dice

**El estado primario de esta matriz es el que `O20` admite, y no se toca.** Lo que cambia es
que **deja de estar solo**: `O20` §1 lo definió como «la obligación está escrita entera, con
propietario, fase, entrada, salida, evidencia, escenarios y criterio de cierre», y eso mide
**que las casillas tengan texto** — no que el texto sea coherente consigo mismo ni ejecutable
con las normas vigentes. El GATE ARQUITECTÓNICO FINAL encontró **dos contratos con todas sus
casillas llenas que no se podían construir**, y con un solo estado no había forma de decirlo.

**La semántica la fija §20.3 del documento 11**, con tres valores —
`CONTRATO_ESTRUCTURALMENTE_COMPLETO` · `CONTRATO_CONSTRUIBLE` ·
`CONTRATO_BLOQUEADO_POR_DEPENDENCIA`— y **vive en la columna `clasificación` de §20.1**, una
por contrato. **Esta matriz no la copia**: la LEE, por la columna `prueba_de_cierre` de cada
fila.

```bash
# la clasificación de cada contrato, en su única sede
grep -oE '^\| `V6-[0-9]+`.*\| (`CONTRATO_[A-Z_]+`)' \
  docs/evolucion/11-ARQUITECTURA-INTEGRADA.md | grep -oE 'V6-[0-9]+|CONTRATO_[A-Z_]+'

# y qué filas de ESTA matriz cuelgan de un contrato BLOQUEADO, derivado y no escrito:
for v in $(grep -E '^\| `V6-[0-9]+`.*CONTRATO_BLOQUEADO_POR_DEPENDENCIA' \
             docs/evolucion/11-ARQUITECTURA-INTEGRADA.md | grep -oE 'V6-[0-9]+'); do
  awk -v v="$v" '/^\| `C-[0-9]+` \|/ && $0 ~ v {print v" ← "$2}' \
    docs/evolucion/CHECKPOINT-ADS-NEXT.md
done
```

**REGLAS, y son las de §20.3 aplicadas a esta tabla:**

- **ningún contrato BLOQUEADO cuenta como construible**, y por tanto una fila cuyo
  `prueba_de_cierre` incluya uno **no puede presentarse como lista para que `F6` la
  construya**: `F6` la construirá cuando la dependencia se levante;
- `CONTRATO_COMPLETO_PARA_F6` **no significa** corregido, ni implementado, ni ejecutado, ni
  certificado, ni construible. Significa lo que `O20` dice que significa, y nada más;
- **ninguna fila cambia de estado primario por esto.** La clasificación semántica es un
  atributo **del contrato**, no del hallazgo, y no se mezclan — la misma disciplina que
  `C-11` estrenó con su atributo secundario.
...
## PARTE DE LA TANDA POSTERIOR AL GATE ARQUITECTÓNICO FINAL — un renglón por hallazgo, y el recuento se DERIVA

> **Misma disciplina que los tres partes anteriores.** Ni una suma escrita: una fila por
> identificador, y el número se saca del árbol.
>
> ```bash
> awk '/^### Lo aplicado por la tanda del GATE ARQUITECTÓNICO FINAL/{t=1} t' \
>   docs/evolucion/CHECKPOINT-ADS-NEXT.md | grep -oE '`H-[0-9]+`' | sort -u | wc -l
>
> # y la COBERTURA contra el gate, que es lo que hay que comprobar: sale VACÍO
> comm -23 <(grep -oE '\*\*`H-[0-9]+`\*\*' \
>              docs/evolucion/30-GATE-ARQUITECTONICO-FINAL-F4C.md \
>            | grep -oE 'H-[0-9]+' | sort -u) \
>          <(awk '/^### Lo aplicado por la tanda del GATE ARQUITECTÓNICO FINAL/{t=1} t' \
>              docs/evolucion/CHECKPOINT-ADS-NEXT.md | grep -oE 'H-[0-9]+' | sort -u)
> ```
>
> **APLICAR NO ES CERTIFICAR.** Ninguno de estos hallazgos se declara SUPERADO.

**LO QUE ESTA TANDA ES, EN UNA LÍNEA.** El diagnóstico de cierre agrupó los hallazgos del
último gate en **OCHO CAUSAS INDEPENDIENTES**, y midió que **sólo cinco de los dieciséis
tocan lo que `F6` necesita para construir** y que **ninguno exige arquitectura nueva**: cuatro
son redacción del contrato y uno es añadir un nodo a un grafo ya decidido. Esta tanda **cierra
cada causa por su propiedad**, no por su instancia: donde había un cardinal escrito lo retira
y publica el comando que lo deriva —nunca lo sustituye por otro—; donde había una obligación
enrutada a un contrato que no la contiene, la lleva a uno que sí; y donde había una
dependencia sin declarar, la declara con su sede, su fase, su propietario y su condición
exacta de desbloqueo.

### Lo aplicado por la tanda del GATE ARQUITECTÓNICO FINAL

| id | sede del remedio | qué se hizo, y con qué prueba |
|---|---|---|
| **`H-01`** | emisor · `CHECKPOINT` matriz · doc 11 §11.6 | **`C-20` vuelve a `F4c`, que es su fase normativa, y el defecto se CORRIGE en vez de contratarse.** `O19` L315-317 enumera SEIS cosas que cada revisor debe recibir externamente y **la primera es el TEXTO de la ratificación**; el sobre llevaba las otras cinco. `V6-16` y `V6-17` **no mencionan el contenido del sobre**, de modo que `F6` podía cerrarlos en verde con el defecto intacto, y §11.6 ya le había dado fase «ya, para el PRÓXIMO gate de `F4c`». El emisor transporta ahora el texto íntegro leído del COMMIT AUDITADO, con su digest, la receta que lo reproduce y una obligación nueva del revisor. **La rama alternativa —que la sede dejara de exigirlo— NO estaba disponible sin el Owner**, y así queda escrito |
| **`H-02`** | doc 11 §18 | **El orden de construcción de `F6` gana el nodo 9**: el verificador y la raíz externa, que `O20` decidió y este grafo no dibujaba, con la arista `9 → 8` que BLOQUEA PesquerApp. **No se reordena nada**: los nodos 0 a 8 conservan orden, dependencias y numeración, y se inserta la dependencia que `O20` y `O18` ya declaraban |
| **`H-03`** | doc 11 §15.4 | `O20` **y** `O21` reciben su fila, que es lo que la propia regla de la sección exige —«UNA FILA POR RESOLUCIÓN del Owner»— y lo que su sanción anunciaba. Comprobado por cobertura: `diff <(filas de §15.4) <(cabeceras `### \`O` del registro)` sale VACÍO |
| **`H-04`** | doc 11 §15.8 | `D109` **y** `D110` reciben su bloque, donde `D107` y `D108` lo abrieron. Cada uno declara **qué reparte y qué NO elige**, que es el objeto de esa serie |
| **`H-05`** | doc 11 §20.1 `V6-19` · `CHECKPOINT` matriz | **La mitad `F6` de `C-11` deja de colgar de `V6-04`**, que contrata otra propiedad —un censo derivado de las lecturas Git—. Se escribe **`V6-19`**, con todos los campos de §20.1: *cada FÓRMULA COMPARTIDA tiene UNA SOLA SEDE, y sus consumidores la IMPORTAN en vez de reescribirla*, con su escenario negativo —una segunda definición da ROJO aunque hoy coincida— y su cierre derivable. **No se declara implementación ni ejecución** |
| **`H-06`** | doc 11 §20.1 `V6-15` · §20.5 | **El escenario negativo pasa a exigir exactamente lo que su ENTRADA entrega**, y el conjunto se DERIVA de los documentos 27, 28 y 29 con el comando que §20.5 publica. Se retira «los once árboles» —los once son de OCHO gates, y el octavo, `DD-01`, es del quinto—. **No se sustituye once por tres**: se retira el cardinal y se remite. La incorporación de los árboles históricos queda como **DEUDA DE INVENTARIO declarada**, con propietario `VER` y fase `F6`, y saldarla hace crecer la entrada **sin editar ningún campo** |
| **`H-07`** | doc 11 §20.1 `V6-16` · §20.4 · §20.3 | **`V6-16` declara su dependencia de `PN-19`**, la enlaza y enlaza §11.8, con fase `F5`, propietario el Owner y **condición exacta de desbloqueo**. Queda **`CONTRATO_BLOQUEADO_POR_DEPENDENCIA`**, no «completo y construible». **No bloquea `F4c`**: la decisión no está oculta —tiene sede, fase, propietario y texto—; lo que faltaba era CITARLA |
| **`H-08`** | doc 11 §11.6 | El campo 17 pasa a exigir **todas las resoluciones que la SEDE contenga**, derivadas por el emisor de las cabeceras `# \`Onn\``, con `O17`·`O18`·`O19` como **mínimo exigido**. Fijar tres donde la sede ya llevaba más era una norma que caduca cada vez que el Owner resuelve, y **el instrumento ya hacía lo correcto** |
| **`H-09`** | `CHECKPOINT` `based_on:` | **Se retiran los DOS restos**: el fragmento cortado a media frase y la segunda columna de una fila cuya primera columna ya no existía, que **COPIABA un estado dentro del campo que declara no copiar estados**. `based_on` se reancla a sus fuentes por SEDE, cada una con el comando que deriva su contenido, y ninguna enumera identificadores |
| **`H-10`** | `CHECKPOINT` `owner_captado:` | «O17, O18 y O19» y «NINGUNA de las tres» se RETIRAN: la sede publica más, y el cardinal caducó dos veces en dos tandas. Se remite a `grep -oE '^# \`O[0-9]+\`'` sobre la sede canónica |
| **`H-11`** | batería · tres rótulos | **SÓLO RÓTULOS.** La regla APPEND-ONLY de `docs/owner/` es de `O19`, que crea la sede con ese contrato; la tanda de `O20` fue la OCASIÓN de la corrección. Uno de los tres se IMPRIME en la salida. **Ni una línea de lógica, ni el perímetro, ni una guarda** |
| **`H-12`** | doc 11 §19 y §2.1 | La familia **`V6-<nn>`** entra en el censo de poblaciones adversariales —la sede definitoria, la que gobierna la prueba de `D83`— y en el inventario de «nada está probado». **Sin cifra**: cada sede publica el comando que deriva la suya |
| **`H-13`** | derivador · `ENCARGO` | **SÓLO EL TEXTO.** Las cláusulas describían el OCTAVO gate mucho después de que cerrara. **Ni una ruta cambia**, y el universo derivado es idéntico |
| **`H-14`** | dec `D109` · `CHECKPOINT` · `00-INDICE` · doc 11 §20 | «once campos cada uno» se retira de las tres sedes: la tabla nunca dio once por fila. Y **la FUENTE global de §20 se ACOTA**, porque `V6-12` deriva de `O19` y de las reglas de la sede, **no de `O20`** — la fila lo dice ahora |
| **`H-15`** | dec | El marcador `> ` incrustado cuatro veces dentro de una línea de cita se corrige. **El contenido resolutivo no se toca** |
| **`H-16`** | `CHECKPOINT` cabecera · `00-INDICE` | «catorce» y «ocho» se retiran de las dos sedes que a la vez declaraban que el recuento era DERIVADO, y se publica el `awk` que lo deriva de la tabla |

### Y el BARRIDO DE CLASE de `C-L.7`, que es lo que esta tanda añade sobre los tres renglones

`H-09`, `H-10` y `H-16` son **tres instancias de una misma clase**, y corregir tres instancias
es lo que este expediente lleva nueve gates haciendo. La clase queda barrida entera:

```bash
# el CONJUNTO DE CAMPOS se deriva del propio bloque, no se escribe
awk '/^```/{f=!f} !f && /^[a-z_]+:/{sub(/:.*/,"");print}' \
  docs/evolucion/CHECKPOINT-ADS-NEXT.md | sort -u
```

Y la garantía queda escrita **DENTRO** del bloque, como **regla 7** de `regla_de_reanclaje`,
...
0  NO HAY SIGUIENTE ACCIÓN       y sigue siendo deliberado. **El método de corrección
   AUTOMÁTICA. EL MÉTODO         iterativa de `F4c` continúa DETENIDO**: la OPCIÓN C se
   SIGUE DETENIDO                activó tras el gate anterior y **el último gate NO la
                                 levanta**. Lo que hubo entre medias fue una tanda de
                                 implementación ACOTADA, autorizada expresamente, y su
                                 VERIFICACIÓN EXTERNA. **La siguiente decisión es del Owner.**

1  QUÉ HA PASADO                 un gate independiente verificó la tanda, se declaró VÁLIDO,
                                 **CERTIFICÓ LA COBERTURA** midiendo las seis condiciones de
                                 `O21` §4 una a una, y devolvió **INSUFICIENTE PARA F5** por
                                 un hallazgo FALLIDO. **El detalle NO se copia aquí** —regla
                                 1—: vive en el documento del gate, que se localiza con
                                   ls docs/evolucion/[0-9][0-9]-*.md | sort | tail -1

2  LO QUE ESTRENA, Y ES LO       **por primera vez en el expediente, las DOS declaraciones
   QUE `O21` VINO A HACER        se emiten POR SEPARADO y la cobertura se CERTIFICA mientras
   POSIBLE                       el veredicto es de insuficiencia**, con prueba contrafáctica
                                 en las dos direcciones. Y **por primera vez los DOS
                                 revisores cierran `ASIGNADO − LEÍDO` = ∅**.

3  QUÉ **NO** SIGNIFICA          **no cierra `F4c`**, que sigue ABIERTA con hallazgos vivos y
                                 el remedio de cada uno escrito · **no autoriza `F5`, `F6` ni
                                 PesquerApp**, que sigue BLOQUEADA · **no declara SUPERADO**
                                 ningún hallazgo, ni `M-04`, ni `C-L.7` · **no revisa ninguna
                                 resolución del Owner** · **y no levanta la OPCIÓN C**.

4  QUÉ QUEDA ENTREGADO           el veredicto literal · el cierre individual de cada hallazgo
                                 del gate anterior · los que quedan vivos, con severidad,
                                 clase, sede fichero:línea, remedio EXACTO y **procedencia
                                 por `git blame`** en cuatro categorías · **las seis
                                 condiciones de cobertura MEDIDAS** y la tupla que las liga.
                                 Quien retome esto no tiene que volver a derivar nada de eso.

5  LO QUE NO CAMBIA              **APLICADA NO ES CERTIFICADA. DETENER NO ES CERRAR.
                                 CERTIFICAR COBERTURA NO ES DECLARAR SUFICIENCIA.** `F4c`
                                 sigue ABIERTA y `F5` sigue NO AUTORIZADA hasta que un gate
                                 independiente diga otra cosa, con esas palabras.
```

## Siguiente acción exacta — HISTÓRICA, anterior al GATE DE VERIFICACIÓN
...
0  DÓNDE ESTAMOS               el OWNER ha resuelto la SEMÁNTICA de `C-L.5`: es `O21`, y su
                               texto vive en la SEDE CANÓNICA. El último gate y su documento
                               NO se escriben aquí —regla 2—:
                                 ls docs/evolucion/[0-9][0-9]-*.md | sort | tail -1
                               y la última resolución del Owner sale de SU sede —regla 3—:
                                 grep -o '^# `O[0-9]*`' \
                                   docs/owner/ADS-OWNER-RESOLUCIONES.md | tail -1

1  QUÉ CAMBIA, Y QUÉ NO        CAMBIA la SEMÁNTICA de `C-L.5`: certifica COBERTURA y nada
                               más, es INDEPENDIENTE del veredicto de suficiencia, y DEJA DE
                               SER DISCRECIONAL. **NO cambian sus SEIS condiciones**, que
                               siguen siendo las del documento 19, `O-04` y `P-08`. **`O21`
                               NO declara suficiente a `F4c`**, NO corrige ningún hallazgo y
                               NO autoriza `F5`, `F6` ni PesquerApp, y lo dice.

2  LO QUE ESTA TANDA HA HECHO  registrar `O21` en la sede canónica —APPEND-ONLY— y
                               proyectarla en `D110`, §15.4 y §15.8; y aplicar los hallazgos
                               del último gate, uno por identificador, en el PARTE DE LA
                               TANDA de este fichero. **Ni un cardinal se escribe aquí**: el
                               recuento y la cobertura contra el documento del gate salen de
                               los dos comandos que ese parte publica.

3  LO QUE NO SE HA HECHO,      **no se ha implementado ni un punto del contrato de `F6`**.
   Y ES DELIBERADO             **No se ha tocado la lógica del verificador de admisión**: de
                               la batería, tres RÓTULOS; del derivador, el TEXTO del encargo.
                               No se ha modificado material protegido, ni el texto de
                               ninguna `O` ni el texto resolutivo de ninguna `D` anterior, y
                               **ningún hallazgo se declara SUPERADO**.

4  QUÉ HAY QUE HACER AHORA     PUBLICAR UNA CANDIDATA NUEVA en una referencia `review/…` que
                               no exista, y someterla a UN GATE INDEPENDIENTE con agentes de
                               contexto limpio que NO sean quien aplicó esta tanda: dos
                               revisores en paralelo y sin verse, y un adjudicador que reciba
                               los dictámenes cerrados, reproduzca cada afirmación contra
                               fichero y línea, **no resuelva por mayoría y no corrija**.
                               Con manifiesto previo conforme a `DD-17` y **SOBRE DE ANCLA**
                               emitido antes de crear a ningún revisor.

5  QUÉ TIENE QUE JUZGAR        los dieciséis hallazgos y las ocho causas · `O21` y la
                               INDEPENDENCIA entre cobertura y suficiencia · `C-L.5` por sus
                               SEIS condiciones · `C-L.7` como CLASE · `C-20` y el CONTENIDO
                               REAL del sobre · `V6-15` · `V6-16` y `PN-19` · la
                               clasificación semántica de TODOS los contratos de §20 · el
                               orden de construcción hasta PesquerApp · que no quede
                               ARQUITECTURA OCULTA · que ninguna deuda de `F6` se presente
                               como implementada · que PesquerApp siga BLOQUEADA · y que no
                               haya REGRESIÓN en `O20` ni en la matriz de los 22.

6  Y EL ADJUDICADOR EMITE      DOS declaraciones SEPARADAS, y `O21` le prohíbe condicionar la
   DOS COSAS, NO UNA           primera a la segunda:
                                 `C-L.5 CERTIFICADA PARA ESTE GATE` **o** `C-L.5 ABIERTA`,
                                 y si es ABIERTA, **nombrando la condición incumplida**
                                 `SUFICIENTE PARA F5` **o** `INSUFICIENTE PARA F5`
                               Las dos pueden coexistir: la primera habla de COBERTURA, la
                               segunda de SUFICIENCIA ARQUITECTÓNICA.

7  LO QUE NO CAMBIA            **APLICADA NO ES CERTIFICADA.** REGISTRAR, PROYECTAR o
                               CONTRATAR no es CORREGIR, y CORREGIR no es SUPERAR. `F4c`
                               sigue ABIERTA y `F5` sigue NO AUTORIZADA hasta que un gate
                               independiente diga otra cosa, con esas palabras.
```

## Siguiente acción exacta — HISTÓRICA, anterior a `O21`
...
2  LO QUE ESTA TANDA HA HECHO  registrar `O20` en la sede canónica —APPEND-ONLY— y
                               ...
                               `F6`** en §20 del documento 11, un punto por obligación y
                               todos los campos que su cabecera define; repartir los 22
                               hallazgos del octavo gate en la MATRIZ DE CIERRE, con un
                               estado primario cada uno; y corregir los que son de `F4c`.
                               **NI UN CARDINAL SE ESCRIBE AQUÍ, y es `H-14`**: este renglón
                               decía «DIECIOCHO puntos y sus once campos cada uno» y
                               «los CATORCE», tres cifras al lado de las tablas que las dan,
                               y la de los campos **nunca cuadró** —la fila lleva sus celdas
                               y la cabecera de §20.1 sus globales—. El detalle está en su
                               sede, con el comando que la cuenta:
                                   grep -cE '^\| `V6-[0-9]+` \|' \
                                     docs/evolucion/11-ARQUITECTURA-INTEGRADA.md
                                   awk '/^\| `C-[0-9]+` \|/{print $4}' \
                                     docs/evolucion/CHECKPOINT-ADS-NEXT.md | sort | uniq -c
...
0  DÓNDE ESTAMOS               el SEXTO GATE DE CERTIFICACIÓN ha devuelto **INSUFICIENTE
                               PARA F5** y **ES VÁLIDO**, por segunda vez consecutiva. Su
                               documento NO se escribe aquí: se deriva con
                               `ls docs/evolucion/[0-9][0-9]-*.md | sort | tail -1`, y su
                               recuento vive dentro de él.

1  QUÉ ENCONTRÓ, EN UNA        **EL NOVENO ÁRBOL, y estaba en el REMEDIO DEL GATE
   LÍNEA                       ANTERIOR.** `DD-02` cerró la guarda de admisión para
                               `docs/owner/` y sólo para esa zona; en el resto del corpus
                               seguía mirando únicamente lo que aún no estaba en `HEAD`.
                               `git add -A && git commit` **sin un solo flag** metía una
                               segunda sede normativa declarando `F4c` cerrada y `F5`
                               autorizada, con `38/38` y el digest anclado bit a bit.

2  LO QUE ESTA TANDA HA HECHO  **el detalle NO se copia aquí: vive en el PARTE DE LA TANDA
                               POSTERIOR AL SEXTO GATE**, una fila por identificador, con
                               el comando que lo cuenta y el que comprueba su cobertura
                               contra el gate. En una línea: **el alcance de la guarda se
                               DERIVA contra la REVISIÓN BASE en vez de ampliarse zona a
                               zona**, que es la única forma de que no haya que repetir
                               esta corrección en el gate siguiente. **Control positivo con
                               las CINCO variantes COMMITEADAS: las cinco dan ROJO.**

3  LO QUE NO SE HA HECHO,      **ni una comprobación nueva**: lo que `EE` pidió era de
   Y ES DELIBERADO             RESTA y de ALCANCE, y así se hizo. No se declara SUPERADO
                               ningún hallazgo, no se cierra `M-04`, y no se edita nada
                               inmutable —`EE-02`, `EE-08` y `EE-14` van al CORRIGENDUM—.
                               Ningún número se sustituye por otro donde se podía RETIRAR
                               y REMITIR.

4  QUÉ VUELVE AL OWNER         **NADA.** `EE` examinó cuatro candidatos a clase `B` y los
                               cuatro caen. Tercera vez consecutiva.

5  QUÉ VIENE AHORA             publicar candidata y encargar otro gate independiente con
                               revisores de contexto limpio que NO sean quien aplicó esta
                               tanda **ni haya participado en ningún gate anterior**. Su
                               commit de manifiesto lleva **el manifiesto, su fila en la
                               LISTA de `00-INDICE.md` y la evidencia derivada
                               reejecutada** (`DD-17`, que ya funcionó dos veces), y su §6
                               **DERIVA las dos aritméticas** con su comando y da **razón
                               publicada de cada fuente sin fila** (`EE-02`).

6  LAS TRES PREGUNTAS QUE EL   · **`M-04` y el DÉCIMO ÁRBOL.** La guarda ya no exime lo
   GATE SIGUIENTE TIENE QUE      confirmado y su alcance se deriva. ¿Queda otra puerta?
   RESOLVER EXPRESAMENTE         Recuerda que `C` NO es exigible dentro de `F4c`.
                               · **`C-L.5`.** Dos gates seguidos han medido las dos restas
                                 a ∅ y ninguno ha escrito CERTIFICADA. **Escríbela o di por
                                 qué no**, como `EE` hizo.
                               · **LAS PROMESAS.** `EE` fijó la condición de salida: «se
                                 cierran instancias y no clases» pasa a deuda registrada
                                 cuando el perímetro se derive **y** las promesas digan lo
                                 que el código hace. **La primera mitad se cumplía ya; esta
                                 tanda va a por la segunda.** Compruébalo.

7  LO QUE SIGUE ABIERTO        `M-04` **NO se declara superada**. `C-L.5` sigue **ABIERTA**.
                               `C-L.7` sigue **NO CERRADA** —reanclar no es certificar—. Y
                               la limitación que `O18` declara de sí misma sigue VIGENTE:
                               eso es el verificador externo de `F6`.
...
9  EL INTÉRPRETE, QUE SE       el runner exige **≥ 3.11** (`tomllib`). Con el 3.10 del
   DECLARA CON SU CIFRA        sistema caen `arranque`, `fuentes` y `workspace`, idénticas
                               sobre `HEAD` sin tocar: es `A14`, limitación aceptada,
                               propietario `PLT`, fase `F6`. Quien publique evidencia
                               **declara su `python3 --version`**.
...
11 QUÉ DEL KERNEL ESTÁ         **NO SE ENUMERA AQUÍ, Y ES LA REGLA.** Vive en el campo
   TOCADO, Y DÓNDE MIRARLO     **EXCEPCIÓN EXACTA DEL KERNEL** de este mismo fichero, y
                               `G-23` lo contrasta contra el árbol. Esta sección REMITE y
                               no copia ninguna ruta: copiarlas es `M-06`, y su
                               reincidencia es `R-02`.
```

> **La regla de redacción de esta sección, que NO es histórica y se conserva:** nada que otra
> sede pueda derivar se copia aquí. **Cuántas veces ha caducado no se escribe: se DERIVA** —
> `grep -c '^## Siguiente acci[óo]n exacta — HISTÓRICA' docs/evolucion/CHECKPOINT-ADS-NEXT.md`—.

> **[HISTÓRICA · «Siguiente acción exacta» anterior al SEXTO GATE DE CERTIFICACIÓN. Se
> conserva para trazabilidad y NO describe el estado vigente.]**

## Siguiente acción exacta — HISTÓRICA, anterior al documento 27

```text
0  DÓNDE ESTAMOS               el QUINTO GATE DE CERTIFICACIÓN ha devuelto **INSUFICIENTE
                               PARA F5** y, al revés que el cuarto, **ES VÁLIDO**. Su
                               documento, su recuento y su clasificación NO se copian aquí:
                               el documento se deriva con
...
1  POR QUÉ ES VÁLIDO,          porque **el SOBRE DEJÓ DE TRANSCRIBIRSE A MANO**. Se emite una
   EN UNA LÍNEA                vez a un fichero FUERA del repositorio auditado y cada revisor
                               lee de ahí; cuatro lo embebieron entero y el adjudicador
                               verificó que los cuatro bloques son byte a byte el fichero.
                               **Divergencia CERO.** La causa exacta que invalidó el cuarto
                               queda cerrada **por construcción, no por diligencia**.

2  QUÉ ENCONTRÓ, EN UNA        **EL OCTAVO ÁRBOL, y lo encontró el ADJUDICADOR, no los
   LÍNEA                       revisores.** El perímetro de exclusión decía excluir «por
                               NATURALEZA» y excluía **por NOMBRE**: un documento que declara
                               `F4c` cerrada y `F5` autorizada, nombrado `<algo>.pyc` bajo
                               `docs/owner/`, alcanzaba el commit, dejaba `git status` vacío,
                               pasaba la batería y el runner enteros, **no entraba en el
                               universo, no recibía fila ni revisor y producía el digest del
                               sobre bit a bit idéntico**. La circularidad **se movió por
                               cuarta vez**, y esta vez estaba en el PERÍMETRO.
                               Y `BB4` nombró la clase: **el sistema cierra INSTANCIAS y no
                               CLASES**.

3  LO QUE ESTA TANDA HA HECHO  **el detalle NO se copia aquí: vive en el PARTE DE LA TANDA de
                               este mismo fichero**, que enumera hallazgo a hallazgo lo
                               aplicado y publica el comando que lo cuenta. En una línea: se
                               cierra la CLASE y no la instancia en los dos instrumentos —el
                               perímetro pasa a ser CONTENIDO y RUTA, la admisión de
                               `docs/owner/` se evalúa sobre el COMMIT, la zona normativa se
                               DERIVA entera, el guardián de los titulares deriva de verdad y
                               la única prueba negativa anclada deja de ser tautológica—, y
                               **cada uno con su CONTROL POSITIVO ejecutado y su salida**.
                               Lo documental va con la misma disciplina: **ningún estado se
                               sustituye por otro donde se podía RETIRAR y REMITIR**.

4  LO QUE NO SE HA HECHO,      **no se ha añadido ni una comprobación nueva al censo**: sigue
   Y ES DELIBERADO             en el número que la batería publica, y quien lo dude ejecuta
                               `python3 docs/evolucion/verificacion/comprobar-correccion-gate-de-cierre.py | tail -3`.
                               No se declara SUPERADO ningún hallazgo, no se cierra `M-04`,
                               no se edita la SEDE CANÓNICA —es append-only—, ni el texto
                               resolutivo de ninguna `O` ni de ninguna `D`, ni ningún
                               dictamen ni manifiesto ya publicado: lo de `DD-18` y `DD-19`
                               va al CORRIGENDUM, que es la vía.

5  QUÉ VUELVE AL OWNER         **NADA.** `DD` examinó CUATRO candidatos a clase `B` y los
                               cuatro caen; es la segunda vez consecutiva que ninguna
                               decisión queda pendiente del Owner, y `DD` lo señala como
                               información buena.

6  QUÉ VIENE AHORA             publicar candidata y encargar otro gate independiente con
                               revisores de contexto limpio que NO sean quien aplicó esta
                               tanda. **Y su commit de manifiesto lleva CUATRO ficheros y no
                               uno** —`DD-17`—: el manifiesto, **su fila en `00-INDICE.md`**
                               y **la evidencia derivada reejecutada**, en el MISMO commit.
                               Cinco gates seguidos dejaron el árbol que juzgaban con un
                               validador canónico en ROJO por commitear el manifiesto solo.

7  LAS DOS PREGUNTAS QUE ESTE  · **`C-L.5`.** El quinto gate midió las DOS restas a **∅** y
   GATE TIENE QUE RESOLVER       escribió «no se reabre por cobertura», **pero NO escribió
   EXPRESAMENTE                   CERTIFICADA**. Esta tanda **no pone esa palabra por él** y
                                 la deja ABIERTA con la medición registrada. **El gate
                                 siguiente emite la palabra o dice por qué no.**
                               · **`M-04` y el octavo árbol.** ¿Hay un NOVENO? El perímetro
                                 se cierra ahora por CONTENIDO y por RUTA, con controles
                                 positivos ejecutados. **Búscale el siguiente**, y recuerda
                                 que `C` —actor privilegiado— NO es exigible dentro de `F4c`.

8  LO QUE SIGUE ABIERTO        `M-04` **NO se declara superada**. `C-L.7` sigue **NO
                               CERRADA**, y el quinto gate le sumó una recurrencia más
                               (`DD-08`). **`C-L.5` sigue ABIERTA**, por lo dicho en 7. Y la
                               limitación que `O18` declara de sí misma sigue VIGENTE: la
                               sede canónica traslada la AUTORIDAD y **no la hace
                               mecánicamente verificable contra una fuente externa al
                               sistema**. Eso es el verificador externo de `F6`.

9  ESTADO, SIN ADORNO          `F4c` sigue **ABIERTA**. `F5` sigue **NO AUTORIZADA**. No se
...
10 EL RUNNER, Y CON QUÉ         el runner de `kernel/operativo/validadores/` da **13/13 en
   INTÉRPRETE SE CORRE         verde, 13 evidencias publicadas, 0 problemas**, y
                               ...
                               `Q-08` exige. **PERO SÓLO CON UN INTÉRPRETE ≥ 3.11.**
                               Con el `python3` por defecto de esta máquina —**3.10**— da
                               **10/13**, y las tres que caen —`arranque`, `fuentes` y
                               `workspace`— caen **por una sola causa**: el tooling necesita
                               `tomllib`. Se comprobó que caen IGUAL sobre el árbol de
                               `HEAD` sin tocar, luego **no las causa ninguna tanda**: es
                               ...
                               **CONSECUENCIA OPERATIVA, y el gate siguiente la hereda:** un
                               ...
                               `T148`/`T159` no suban a la capa de certificación como
                               defecto del producto.

11 DÓNDE PARAR                 antes de dar por cerrado lo que sólo un gate independiente
                               puede cerrar. Y sigue vigente parar antes de redactar `(g)`,
                               antes de crear `C8`, antes de tocar `C7` o el kernel
                               operativo SUSTANTIVO, y antes de iniciar PesquerApp.

12 QUÉ DEL KERNEL ESTÁ         **NO SE ENUMERA AQUÍ, Y ES LA REGLA.** La lista exacta de lo
...
> **[HISTÓRICA · «Siguiente acción exacta» anterior al QUINTO GATE DE CERTIFICACIÓN. Se
> conserva para trazabilidad y NO describe el estado vigente.]**

## Siguiente acción exacta — HISTÓRICA, anterior al documento 26
