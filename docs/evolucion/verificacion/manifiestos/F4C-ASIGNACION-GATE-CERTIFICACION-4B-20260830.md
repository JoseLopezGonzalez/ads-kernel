...
>
> **Y emitido DESPUÉS de cerrar el instrumental, que es la novedad.** El primer intento de
> este gate publicó su manifiesto antes de terminar de tocar `verificacion/`, y el emisor
> creció siete líneas después: el manifiesto declaraba una cifra de líneas que ningún árbol
> daba. **Es `U-02` y su reincidencia `X-06`, que dos gates consecutivos han castigado**, y
> esta vez **lo detectó el propio sobre antes de que existiera ningún revisor**, porque
> publica los dos árboles y las rutas en que difieren. Aquel intento queda publicado como
> historia en `review/f4c-gate-certificacion-4-20260830` y `…-4-emisor-…`, no se mueve, y su
> manifiesto **no se repartió a nadie**.
...
FECHA              2026-08-30
RAMA DEL GATE      gate/f4c-certificacion-4b-20260830, creada en ese commit exacto
REGLA QUE ESTRENA  el gate NO toca `verificacion/` después de publicar el manifiesto. Es la
                   segunda de las dos salidas que el adjudicador `X` dejó escritas, aplicada
                   generalizada: el instrumental se cierra ANTES
QUÉ TRAE           la RATIFICACIÓN `O19` del Owner y la SEDE CANÓNICA; la atribución
                   corregida en doce sedes SIN retirar contenido; el sobre anclando la sede
                   con los digest de `O17`, `O18` y `O19`; el inventario de integridad
                   extendido a `docs/owner/`; y el emisor reparado según el tercer gate
...
```text
UNIVERSO DERIVADO   70 fuentes · 58796 líneas   —sobre el árbol del GATE—
QUÉ ENTRÓ NUEVO     el documento 24 por (iv); la SEDE CANÓNICA y el manifiesto del gate
                    anterior por (v)
```
...
ADJUDICADOR AA  recibe los dos dictámenes YA CERRADOS y los dos sobres que los revisores
              ...
              **declara INVÁLIDO el gate ante cualquier diferencia**. No resuelve por
              mayoría. No corrige nada. **Su marca lleva dos letras porque el alfabeto de
              una se agotó**, y el emisor lo acepta sin ablandar su regla.
...
**El coste de las cadenas, declarado:** el lote de lectura son **28515 líneas**, y sólo el
...
Todo derivado del árbol del gate, nada copiado. **Cada fila declara el árbol contra el que
se contrasta**, y el sobre publica las rutas en que los dos árboles difieren.
...
| 7 | `docs/evolucion/verificacion/README.md` | 318 | `01d1ed27c0a2d375d2711072f3dfdf6bc6013d73f8f2a70250cc9f5fcf1d4f88` | v | **Z** | Z1 |
...
| 10 | `docs/evolucion/verificacion/emitir-sobre-de-ancla.py` | 555 | `a98367bd3ff32ceda5a6b8a240032edd03c467f152f373e2e77bc290c64fe1f7` | v | **Z** | Z2 |
...
| 23 | `kernel/operativo/capacidades/CNS/CAPACIDAD.md` | 107 | `e0f79e6c3a467302c3d16aa5ee4ccb45583daf146d1676b2b06995419656c5d5` | ii | documento **22**, L1590 · árbol `4d231ee` |
...
LÍNEAS OBLIGATORIAS     58796

ASIGNADAS A LECTURA         16     28515 líneas · lotes complementarios de `Y` y `Z`
...
· `M-04` COMO PROPOSICIÓN GENERAL. El gate anterior encontró SEIS árboles en verde, dos por
  puertas nuevas. **Busca el séptimo.** Y respeta la distinción que `O18` fija: `C`
