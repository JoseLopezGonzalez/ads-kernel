...
**LA TABLA ESTÁ VACÍA, y es lo que debe estar.** Los cambios de la tanda del documento
24 quedaron confirmados en el mismo commit que vació esta sección. Ningún fichero de
`verificacion/` difiere de `HEAD`.
...
| `G-21` | **las resoluciones del Owner que `7e99388` publica siguen en el registro**, con su texto, sin duplicar y **sin DESAPARECER**, y falla CERRADO sin Git. Borrar la fila `\| O5 \|` daba `38/38` verde e imprimía «ninguna difiere»: la guarda no comparaba lo que no estaba, y el cardinal que hacía de red tampoco lo veía porque **el registro crece** —17 ≥ 16 se cumple con una resolución menos— | trazabilidad · `A1` |
...
Y NO LO PRETENDE            contra referencias que también viven ahí. §11.4 del documento 11
                            ...
                            decisión al Owner, y su RATIFICACIÓN sigue pendiente. **Esta
                            tanda no ha escrito ninguna comprobación nueva —el censo sigue
                            en 38—: ha ensanchado perímetros y ha retirado un interruptor.
                            La tanda ANTERIOR sí escribió una, `G-34`, y este README lo
                            negaba**
