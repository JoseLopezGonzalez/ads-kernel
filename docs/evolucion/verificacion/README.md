...
> **`Z1-06`.** Aquí vivían DOS párrafos consecutivos que se contradecían: «**LA TABLA NO
> ESTÁ VACÍA: la tanda de `O19` está EN CURSO**» y, dos líneas después, «**LA TABLA ESTÁ
> VACÍA, y es lo que debe estar**». El primero era el rótulo de la tanda anterior, que
> sobrevivió al commit que vació la tabla. **Se retira el que había caducado**, y queda uno
> solo: el estado de esta sección lo dice la TABLA, y quien quiera contrastarlo ejecuta
> `git status --porcelain -- docs/evolucion/verificacion/`.

**LA TABLA NO ESTÁ VACÍA: la corrección posterior al CUARTO GATE está EN CURSO.** Los
ficheros de abajo difieren de `HEAD` porque esta tanda los está corrigiendo, y la declaración
**se retira en el mismo commit que confirma los cambios**. Si sobrevive a ese commit, `G-34`
lo dice.

**LA TABLA ESTÁ VACÍA, y es lo que debe estar.** Los cambios de la tanda posterior al
CUARTO GATE quedaron confirmados en el mismo commit que vació esta sección. Ningún fichero
de `verificacion/` difiere de `HEAD`, y `G-34` recupera su tercera dimensión sobre los cinco.
