...
es el comportamiento correcto, y la batería da menos que su total. **No es un defecto: era
que ninguna sede lo acotaba**, y quien leyera «38/38 en verde» podía creer que eso
certificaba el commit desnudo.

**El censo NO se escribe aquí: la batería lo DERIVA de los títulos de sus propias
comprobaciones y lo publica en la última línea de su informe**, con la lista de
identificadores. Quien quiera contrastarlo:

```bash
python3 docs/evolucion/verificacion/comprobar-correccion-gate-de-cierre.py | tail -2
# y la medición, que tiene que coincidir con lo derivado:
rm -rf /tmp/sin-git && mkdir -p /tmp/sin-git && tar -c --exclude=.git . | tar -x -C /tmp/sin-git
cd /tmp/sin-git && python3 docs/evolucion/verificacion/comprobar-correccion-gate-de-cierre.py | tail -3
```

**Qué significa su ausencia:** un árbol sin `.git` **no puede** demostrar que lo histórico
está intacto, que lo normativo no se ha tocado, que ningún dictamen ha cambiado de
veredicto ni que el instrumental es el publicado. Puede demostrar todo lo demás. **Un verde
sobre el árbol desnudo NO es el verde que un gate certifica**, y por eso el sobre de ancla
publica el commit y no sólo el árbol.
...
> **`Z1-06`, y su TERCERA VIDA cerrada por `DD-15` del QUINTO GATE.** Aquí vivían DOS
> párrafos consecutivos que se contradecían: «**LA TABLA NO ESTÁ VACÍA…**» y, dos líneas
> después, «**LA TABLA ESTÁ VACÍA, y es lo que debe estar**». El remedio de `Z1-06` dijo
> «**se retira el que había caducado, y queda uno solo**» — **y quedaron los dos**, que es
> lo que `DD-15` midió. **Instancia corregida, clase abierta: es el modo de fallo del
> expediente entero.**
>
> **Por eso ahora no queda NINGUNO.** Mientras el estado de esta sección se escriba en
> prosa, cada tanda tiene que acordarse de reescribirlo, y una de cada dos no se acuerda.
> **El estado lo dice la TABLA de abajo, y sólo ella**; quien quiera contrastarlo contra el
> árbol ejecuta:
>
> ```bash
> git status --porcelain -- docs/evolucion/verificacion/
> ```
>
> **Tabla con filas** = hay instrumental en corrección, y son exactamente esas rutas.
> **Tabla vacía** = no hay ninguno, y `G-34` exige entonces que todo `verificacion/` sea
> idéntico a `HEAD`. No hace falta ningún rótulo que lo repita, y por eso no lo hay.
...
>   venir vacío. Un sobre sucio no se emite. **Y eso es TODO lo que esa negativa prueba, y
>   así lo dice hoy el sobre** (`Z2-01`≡`Z-11`): que no había modificaciones VISIBLES para
>   `git status` al emitir. Decía además que garantizaba «que el emisor y el derivador que
>   están corriendo son los publicados», y **era falso** —`git status` compara contra el
>   `HEAD` LOCAL, que el emisor no contrasta contra ningún commit, y
>   `git update-index --skip-worktree` lo vacía con el fichero modificado en disco—. La
>   frase se retira; lo que sustituye a la garantía es un CAMPO NUEVO que sí se puede
>   contrastar: el **SHA-256 del EMISOR en los dos commits y el del fichero que ha corrido**.
>   El sobre publicaba el SHA del DERIVADOR dos veces y **nunca el suyo**.
> · **Lee cada byte del COMMIT**, con `git show <commit>:<ruta>`, y ejecuta **el derivador
>   de ESE commit** sobre el árbol materializado con `git read-tree` + `git checkout-index`
>   en un índice temporal fuera del repositorio. Ni una lectura del directorio de trabajo de
>   quien emite. Antes `_universo()` leía con `io.open(RAIZ/ruta)` y el digest publicado no
>   correspondía a ningún commit; y hasta este gate el árbol se desplegaba con
>   `git archive`, **que HONRA `export-ignore`** (`Z2-03`≡`Z-04`): un `.gitattributes`
>   confirmado en el propio commit retiraba un dictamen nuevo del universo, con `rc=0` **y
>   con el digest que el revisor recalcula CUADRANDO**, porque la RECETA usaba el mismo
>   camino. Se ha quitado el paso que mentía; la RECETA publica los mismos dos comandos.
> · **Exige la IDENTIDAD del emisor y rechaza un ROL** (`AA-05`). `--emisor` era texto libre,
>   no obligatorio y **con un rol como valor por defecto**; §11.6 pide «quién lo emite,
>   NOMBRADO» y declara que **el ancla de `O18(b)` ES esa identidad** mientras no exista
>   `(c)`. Sin nombre no hay sobre.
> · **Copia al sobre lo que el componente (iv) del derivador DEJA FUERA**, con su H1 y por
>   los dos commits: un universo que encoge lo dice, y lo dice en el ancla (`Z-08`, `Z-13`).
...
>
> **Y en la corrección POSTERIOR AL CUARTO GATE tampoco.** El adjudicador `AA` volvió a
> ordenarlo —«lo que falta es de RESTA y de DISCIPLINA»— y el censo **sigue en 38**. Lo
> hecho es, dicho entero: **cambiar discriminantes** (`basename` → RUTA COMPLETA en `G-29`),
> **cambiar perímetros de enumerados a derivados** (`docs/owner/` y `manifiestos/` en el
> derivador; `__pycache__` deja de excluirse por su NOMBRE y el bytecode se excluye por su
> EXTENSIÓN; los nombres con punto inicial dejan de saltarse en dos inventarios),
> **hacer que un fixture insatisfacible invoque de verdad al evaluador que dice probar**
> (`G-31c`), **quitar el paso que honraba `export-ignore`** en el emisor, y **retirar o
> acotar las afirmaciones falsas** del instrumento. Ni un `check()` nuevo.
...
| `G-10` | las extensiones de ficha se **DERIVAN de la enumeración `capacidades/<CAP>/` de §5.2**, y de ahí salen el cardinal en letra, el cardinal en cifra y los nombres que §5.2, §16 y §17 tienen que publicar. **`DD-12`: esta comprobación NO DERIVABA NADA** —tres substrings literales y seis nombres escritos a mano— y era **el aval con el que §0 se permite escribir uno de los pocos cardinales que el documento se permite**: una SÉPTIMA extensión dejaba «Son SEIS» caducado **con `G-10` en verde**, que es el modo de fallo exacto que la regla de titulares persigue. Control positivo ejecutado: una séptima ficha en §5.2 da hoy **ROJO en las tres sedes a la vez**, nombrándola. Ningún cardinal se escribe ya en esta comprobación | `I-06` · `DD-12` |
...
| `G-22` | **el inventario de inmutables se DERIVA del árbol** —todo documento numerado de `docs/evolucion/`, todos los manifiestos y, desde `O19`, **`docs/owner/` ENTERO**, menos los cuatro ficheros en corrección— y cada uno se contrasta contra `HEAD` **y** contra la revisión base. No hay ningún rango escrito. Un documento numerado nuevo **nace ADMITIDO por `G-29` y queda protegido aquí en cuanto se confirma**; y una salida de `git ls-tree HEAD` **vacía con éxito** ya no se interpreta como «nada cambió»: falla CERRADO. El detalle dice cuántos inmutables nacieron después de la revisión base y por tanto se contrastan sólo contra `HEAD`. **La zona `docs/owner/` entra en ESTE inventario y no en uno nuevo**: mismo id, mismo doble contraste, mismo perímetro derivado, y el directorio se barre ENTERO en vez de por extensión. Hasta `O19` nadie miraba su CONTENIDO —`G-29` veía nacer y desaparecer ficheros, y editar el texto de una resolución del Owner no lo veía nadie—, y desde `O19` esa zona es la AUTORIDAD CANÓNICA: material del Owner que **no puede alterarse en silencio**. **Y la sede es APPEND-ONLY, con lo que eso implica aquí, dicho para que no sorprenda:** una entrada nueva del Owner deja la sede MODIFICADA respecto de `HEAD` y `G-22` la da en ROJO hasta que se confirma. Es lo que se quiere —material del Owner sin confirmar no pasa por vigente en silencio— y quien la añada responde de ello en el mismo commit. **El barrido de `docs/owner/` saltaba los nombres que empiezan por punto, directorios y ficheros, sin motivo escrito (`Z1-07`); hoy excluye lo mismo que el resto de la batería y nada más.** **Y el detalle NOMBRA a sus EXENTOS** (`AA-03`): los cuatro ficheros EN CORRECCIÓN quedan fuera de este inventario, y uno de ellos es `00-INDICE.md`, **que es la sede que gobierna qué se admite en `docs/owner/` y qué documento numerado nuevo se admite**. Editar sólo `00-INDICE.md` sigue dando `38/38` y ahora el informe dice que no se ha contrastado. **Que esa exención CADUQUE, como caduca la de `G-34`, es una decisión de la tanda que la declara y NO del instrumento: queda ABIERTA y dicha** | `Q-23` · `Q-26` · `T-05` · `O19` · protecciones 1 y 7 |
| `G-23` | lo normativo intacto y la excepción del kernel **contrastada contra la prosa del checkpoint**, que ya no enumera nada a mano; y el punto de entrada **remite** en vez de copiar. **`DD-04`: «lo normativo intacto» se afirmaba sobre un regex de SEIS patrones que cubría 4 de los 14 ficheros de `docs/rediseno/`**, la zona que el `00-INDICE` declara ESPECIFICACIÓN NORMATIVA VIGENTE. El adjudicador lo midió con su control positivo: `00-MAPA.md` reescrito **declarando que prevalece sobre el material APROBADO y deja `O17`·`O18`·`O19` sin efecto** daba **38/38 VERDE**; el mismo texto en `a-CAPACIDADES-APROBADA.md` daba `37/38`. **Hoy la zona se DERIVA del árbol —entera— y lo que se enumera es su EXCEPCIÓN, con motivo**, igual que ya se hacía con el kernel: un fichero nuevo en `docs/rediseno/` nace protegido, y con el regex nacía libre. Control positivo reejecutado: el mismo `00-MAPA.md` da hoy **ROJO** | alcance · `M-06` · `R-02` · `DD-04` |
...
| `G-26` | recuentos derivados en cuatro planos, con las citas históricas reconocidas por **etiqueta estructural de región** y no por una palabra en la línea; y la región **cierra al terminar el bloque que la etiqueta encabeza** —encabezado, valla de código, línea en blanco **también dentro de una valla**, salida de la cita, o **la marca siguiente**—, de modo que una etiqueta ya no exime lo que queda **fuera de su bloque**. **CORREGIDO por `DD-15` del QUINTO GATE, y la enumeración de arriba estaba incompleta:** la LÍNEA EN BLANCO **no cierra la clase `tanda`** —una clave de tanda abarca su bloque entero, blancos incluidos—, y lo que la cierra es **la siguiente línea que empieza en columna 0**. El código lo hacía y este README no lo decía, que es el defecto: la región de clase `tanda` **está acotada** —`CC3` lo demostró contra la «longitud NO ACOTADA» de `CC1-05`, y `DD` le dio la razón—, pero **por otra frontera**. Quien contrastara este renglón contra el código encontraba una promesa que el código no cumple **en la dirección que importa**, y el remedio es decir lo que el código hace. La excepción `not en_valla` dejaba exenta de todo control de cifra **la mitad larga del checkpoint sin que nadie atacara nada**, y se retiró. **`Z1-07`: aquí había DOS CIFRAS ESCRITAS A MANO** —«1 899 de 3 562 (53 %) … bajan a 1 229»— medidas sobre un árbol que ya no existe y que **ninguna comprobación contrastaba**; un revisor las volvió a medir y no daban. **Se retiran, y la cifra vigente la DERIVA `G-26` sobre el árbol que se tiene delante y la publica en su detalle**: ejecute la batería y léala ahí. Ninguna cifra de exención se escribe ya en este README. **Y la tercera rama de `_es_cita`, la de los VERBOS, se retiró entera**: «decía », «en vez de » o «reanclado » delante de una cifra viva falsa apagaban esta comprobación, y ninguna comprobación puede depender de una palabra. Quedan las dos formas ESTRUCTURALES de citar: «…» y la etiqueta de región | higiene · `P-01` · `Q-04` · `Q-07` · `P-05` · `T-06` · `A3` · `A4` · protección 2 |
...
| `G-29` | **topología y unicidad de TODO el corpus gobernado, CONFIRMADO O NO — `EE-01`.** La guarda de admisión **se evalúa contra la REVISIÓN BASE y no contra `HEAD`**: es AMPLIACIÓN todo lo que existe hoy y no existía en la base, esté commiteado o no, y **confirmar un fichero NO lo exime de la condición de su zona**. Hasta el sexto gate la guarda miraba sólo `_disco − _publicado`, es decir, lo que aún no estaba en `HEAD`, **salvo en `docs/owner/`**: `DD-02` cerró ESA zona y sólo ESA. Con eso, `git add -A && git commit` **sin un solo flag** añadía en `docs/normativa/`, en `docs/` o en la RAÍZ una segunda sede normativa que declaraba `F4c` cerrada y `F5` autorizada, dejaba `git status` vacío, pasaba **38/38**, no entraba en el universo obligatorio ni en `EXCLUIDOS_PERIMETRO` ni en `EXCLUIDOS_IV`, no recibía fila ni revisor y **producía el digest del sobre BIT A BIT idéntico al anclado**. Es EL NOVENO ÁRBOL, lo encontró un revisor y **lo reprodujo el adjudicador dos veces**. **Control positivo reejecutado tras el remedio: las CINCO variantes —`docs/normativa/`, la raíz, `docs/`, un documento numerado con ordinal libre sin enlace, y un manifiesto sin enlace— dan hoy ROJO estando COMMITEADAS.** Y las zonas mantienen su condición, que sigue siendo la que su sede escribe: `docs/owner/` y los documentos numerados, enlazados por RUTA COMPLETA desde `00-INDICE.md`; los manifiestos, **enlazados desde la LISTA** —que es lo que `C-L.5` exige y lo que hasta ahora no tenía condición ninguna—; el instrumental, enumerado en este README; y **`docs/canonico/`, el CORPUS CANÓNICO VIGENTE que el Owner ordenó consolidar antes de `F5`, enlazado por RUTA COMPLETA desde `00-INDICE.md`** —zona NUEVA, admitida CLASIFICADA y con esa única condición, por la misma razón y con la misma disciplina con que `O19` obligó a clasificar `docs/owner/`: sin zona, la ampliación que la propia orden manda publicar sería ROJA. **No relaja ninguna condición existente y no exime nada**; admite además `.yml` y `.py` porque el registro de sedes y su validador viven ahí y tienen que estar enlazados igual que los documentos. **Y el ordinal deja de exigirse LIBRE y pasa a prohibir la COLISIÓN**: lo que crea dos sedes con la misma identidad son dos documentos con el mismo número, no la existencia del que ya está. **Y el perímetro de exclusión YA NO SE ESCRIBE: es el repositorio ENTERO menos `.git` y el bytecode**, con la exclusión declarada y con motivo. Era una lista de siete zonas, y una lista escrita caduca: copias **byte a byte** del material APROBADO y del registro en `docs/normativa/` —un directorio que no existía— y en la raíz de `docs/` daban **38/38 verde**, mientras el mismo gemelo dentro de una zona listada daba `FALLO G-29`. La zona normativa para los marcadores canónicos también se deriva: **todo el corpus menos `docs/evolucion/`**, donde el bloque es CITA. Ninguna ampliación sin clasificar, ningún gemelo byte a byte, y ningún marcador de bloque canónico —derivado, no escrito— con sedes nuevas. Un documento numerado nuevo **ya no se admite en blanco**: tiene que estar **enlazado desde `00-INDICE.md`** —la regla que el propio índice escribió— y **no repetir ordinal**. **`docs/owner/` no admitía NINGUNA ampliación**, y `O19` ordenó crear ahí la sede canónica: se admite con UNA condición, que es la que el código ejecuta y toda la que ejecuta — **la RUTA COMPLETA enlazada desde `00-INDICE.md`** —, de modo que una segunda sede plantada en esa zona, al lado o en un subdirectorio, sin ese enlace es ROJA, y su contenido queda después bajo `G-22`. **`Z1-01`≡`W-04`≡`Z-02`: la admisión comparaba `rel.split("/")[-1]`, sólo el `basename`**, y con eso una segunda sede canónica del Owner en `docs/owner/vigente/` o en `docs/owner/x/y/z/` —declarando PREVALECER y dejar `O17`·`O18`·`O19` sin efecto— pasaba en **38/38** mientras `G-22` la NOMBRABA en su detalle imprimiendo `OK`; la línea nació en el commit que implementa `O19`. Hoy la admisión es por RUTA. **Y `AA-03`: aquí se declaraba además «en el mismo commit que la crea», y esa condición NO ESTABA IMPLEMENTADA ni podía estarlo en ese punto** —la rama sólo se consulta para lo que aún no está en `HEAD`—; se retira la declaración en vez de adornarla, y la regla del índice sigue siendo la del índice. **El perímetro de exclusión, dicho como el código lo ejecuta —`DD-03` y `DD-01`—:** se excluye `.git` **ANCLADO A LA RAÍZ** de la ruta relativa, con la poda de `os.walk` evaluada **sobre la RUTA COMPLETA** y no sobre el nombre desnudo del directorio, y el bytecode **POR SU CONTENIDO** —cabecera de CPython y no-texto—, **no por su sufijo**; y **todo lo excluido se PUBLICA con su ruta y su motivo** en el detalle de esta comprobación. **Esta fila decía antes «`.git`, que no es corpus, y el bytecode por su EXTENSIÓN», bajo el titular «el perímetro YA NO SE ESCRIBE… es hoy uno solo y por NATURALEZA», y era FALSO en las dos mitades**: `.git` se excluía por nombre y a cualquier profundidad, y una extensión es un nombre, no una naturaleza. `DD-01`, EL OCTAVO ÁRBOL, lo midió: un documento que declara `F4c` cerrada y `F5` autorizada, nombrado `<algo>.pyc` bajo `docs/owner/`, **alcanzaba el commit, dejaba `git status` vacío, pasaba 38/38 y 13/13, no entraba en el universo, no recibía fila ni revisor, no aparecía en `EXCLUIDOS_IV` y producía el digest del sobre bit a bit idéntico**. Hoy ese mismo fichero da **ROJO** —control positivo ejecutado—, entra en el universo y recibe fila. Antes de eso, `Z1-03`≡`Z-05`: se excluía el DIRECTORIO `__pycache__` por su nombre, con el motivo escrito «los artefactos de bytecode, que no son fuente», y un directorio no es un artefacto de bytecode: dentro vivían en **38/38** una copia byte a byte del material APROBADO, una segunda sede con su propio bloque `ads:proceso` y una copia del EMISOR DEL SOBRE. **Y `DD-02`: la ADMISIÓN de `docs/owner/` se evalúa sobre el CONTENIDO DEL COMMIT.** La guarda del enlace era INERTE sobre todo fichero ya en `HEAD` —`_disco − _publicado`—, de modo que la misma segunda sede daba `37/38` sin commitear y **`38/38` commiteada**, y el objeto que un gate audita es un COMMIT. Hoy la zona se contrasta contra `git ls-tree -r HEAD docs/owner/`, ruta a ruta | `Q-02` · `M-04` · `T-03` · `O19` · `DD-01` · `DD-02` · `DD-03` · protecciones 5 y 9 |
...
| `G-31` | **ninguna comprobación se apaga escribiendo una palabra ni una etiqueta fuera de sitio**: las palabras gatillo se pegan al mismo dato y se exige que recuentos, polaridad, comparación de estado y marca histórica no cambien de veredicto; dos fixtures exigen que una etiqueta **no exima una línea que está fuera de su cita ni el bloque siguiente al suyo**; y **los once VERBOS RETIRADOS de `_es_cita` se prueban uno a uno, puestos DELANTE de la cifra**, que es donde apagaban. La lista de gatillos eran las palabras de `T-06` y **ninguna de las que apagaban de verdad**: la comprobación que existe para cazar interruptores léxicos no probaba el que había. **`Z1-04`≡`Z-06`: el bloque del contraste de ESTADO no invocaba NADA.** Eran dos `if` sobre f-strings comparadas con un literal —`if f"CERRADA {_w}" in ("CERRADA",)`—, **insatisfacibles por construcción**, y el detalle publicaba «× 4 evaluadores» ejercitando tres; se revirtió `G-16c` de igualdad exacta a `startswith` —la regresión que ese bloque existe para cazar— y `G-31` siguió en `OK`. **Es un fixture que no puede fallar dentro de la comprobación cuya tesis es que ninguno puede serlo.** Hoy el contraste de estado vive en `_estado_casa()`, con UNA sede, y las dos lo llaman: `G-16c` para juzgar el árbol y esto para ejercitarlo, con su mitad contraria —el estado limpio SÍ casa— para que no vuelva a ser un «siempre falso». **Y el número de evaluadores ya no se escribe: cada bloque registra el que invoca y el detalle publica lo que se ejecutó** | protección 10 · `T-06` · `A3` |
