...
> **RECONCILIACIÓN DE ESTE PREÁMBULO CNS LO QUE DE VERDAD SE HIZO — `V3-02`+`V3-03`
...
**Epígrafe propio, y es `HH2-09`.** `D108`, `D109` y `D110` estaban archivadas bajo el
encabezado de `D107`, cuyo preámbulo describe **otro gate, otro adjudicador y otro recuento**.
Es exactamente lo que `S-19` denunció para `D107` en su día: **quien buscara «qué decidió la
tanda de `O18`» por los epígrafes no la encontraba.** `S-19` se cerró MOVIENDO `D107` y no
estableciendo la regla, y por eso las tres siguientes volvieron a caer en la misma estructura.

**LA REGLA, escrita ahora para que no haya una cuarta vez:** *toda propagación de una
resolución del Owner abre EPÍGRAFE PROPIO en este registro, con su origen, su resolución y su
fecha, igual que ya lo tiene en §15.8 del documento 11.* Una fila de propagación bajo el
epígrafe de otra tanda es el defecto, no la excepción.

**Origen:** el **SEGUNDO GATE DE CERTIFICACIÓN** —documento 23— devolvió su única clase `B`,
que no era un hallazgo sino la raíz de tres insuficiencias consecutivas. El Owner resolvió, y
su resolución es **`O18`**, cuyo texto íntegro vive en la **SEDE CANÓNICA**
[`docs/owner/ADS-OWNER-RESOLUCIONES.md`](../owner/ADS-OWNER-RESOLUCIONES.md). La fila de abajo
es su PROYECCIÓN DERIVADA y no la sustituye.
...
**Epígrafe propio por la regla de arriba.** **Origen:** el **OCTAVO GATE DE CERTIFICACIÓN**
—documento 29— devolvió INSUFICIENTE, y su adjudicador midió que la causa raíz no era ninguno
de sus hallazgos: era que `F4c` estaba obligada a acreditar la IMPLEMENTACIÓN del verificador
que sólo `F6` puede construir. El Owner resolvió por su cuenta, y su resolución es **`O20`**,
del **2026-09-01**, cuyo texto íntegro vive en la **SEDE CANÓNICA**. Su contrato ejecutable es
**§20 del documento 11**, y su bloque propio, **§15.8** del mismo documento.

| # | decisión | qué revisa | por qué, y qué alternativa se descartó | cómo se revierte |
|---|---|---|---|---|
| D109 | **PROPAGACIÓN DE `O20`. No es una elección de F4c: es la materialización de una resolución del Owner, y se declara DERIVADA. No sustituye a `O20`.** (i) **La FRONTERA DE FASE queda escrita**: `F4c` produce la arquitectura —invariantes, contratos, propietarios, fases, entradas y salidas, condiciones de fallo cerrado, matrices adversariales, criterios de aceptación ejecutables y asignación inequívoca de cada obligación a `F5` o `F6`—, y **NO tiene que demostrar que la implementación provisional del verificador interno satisfaga esos contratos**. (ii) **`F6` recibe NUEVE responsabilidades nombradas**: implementar el verificador de admisión, implementar la raíz externa de confianza de `O18`/`O19`, cerrar las lecturas Git seguras, comprobar mutaciones de ficheros nuevos y preexistentes, tratar codificaciones no UTF-8, cubrir adición, modificación, borrado, renombrado y cambio de tipo, impedir que la definición de lo verificado y la regla de admisión se excluyan a sí mismas, ejecutar la matriz adversarial completa, y **certificar la implementación antes de declarar ADS operativo o iniciar la adopción permanente de PesquerApp**. (iii) **El CONTRATO OBLIGATORIO DE `F6` se escribe completo y sin implementar**, en §20 del documento 11: un punto por obligación —**cuántos son NO se escribe aquí**, `H-14`: lo da `grep -cE '^\| `V6-[0-9]+` \|'` sobre §20.1— con los atributos que su tabla lleva **por fila** —qué debe demostrar, entrada, salida, evidencia, escenario positivo, escenario negativo, condición de bloqueo, criterio exacto de cierre y **clasificación**— y los que la cabecera de §20.1 declara **globalmente** —propietario global, implementador, dosier, bloqueo por seguridad, autoridad de aceptación y fase de ejecución—. Esta fila decía «once campos cada uno» y **la tabla nunca dio once por fila**, además de atribuir a todos una FUENTE que §20 declara en cabecera y que **no es `O20` para `V6-12`**. **Ninguno se ha ejecutado, y así se dice.** (iv) **La batería interna se declara EVIDENCIA DE CONSISTENCIA DEL CORPUS y no raíz autosuficiente de certificación de su propia implementación**, y queda escrito que un verde suyo **NO demuestra** que el verificador de `F6` esté construido ni certificado. (v) **Los 22 hallazgos del octavo gate reciben MATRIZ DE CIERRE POR FASE**, con un estado primario por hallazgo y atributos secundarios separados; **ninguno se declara SUPERADO**, y los que permiten un falso verde **bloquean `F6` y PesquerApp**. (vi) **PesquerApp queda BLOQUEADA sin MVP, sin piloto desechable y sin adopción parcial** hasta que `F6` implemente y certifique | **`O20`**, que es su única fuente. No revisa ninguna decisión de F4c y **no renumera nada**: `D1`–`D108` y `O1`–`O19` intactas | la FRONTERA DE FASE CIRCULAR que el Owner detecta tras dos gates consecutivos: `F4c` bloquea a `F5`, `F5` precede a `F6`, y `F6` es quien construye el verificador cuya implementación se le exigía a `F4c`. **Las alternativas NO las descartó F4**: las ponderó el Owner, y rechazó expresamente rebajar la exigencia —los contratos siguen siendo obligatorios— y detenerse. Lo que F4 aporta aquí es **exclusivamente el reparto de la decisión por las sedes vigentes y la escritura de los contratos**, y por eso esta fila se declara derivada | retirar `O20` devuelve la recursión: una fase que sólo puede cerrarse demostrando lo que la fase siguiente tiene que construir, que es el estado que los gates séptimo y octavo midieron |

### `D110` · la propagación de `O21` — DERIVADA de una resolución del Owner, NO elegida por F4

**Epígrafe propio por la regla de arriba.** **Origen:** el **GATE ARQUITECTÓNICO FINAL**
—documento 30— dejó a la vista, sin que fuera uno de sus hallazgos, que la regla de cierre de
`C-L.5` es una EXCLUSIÓN de una sola dirección, de modo que dos adjudicadores consecutivos
pudieron actuar de forma opuesta sin incumplir norma alguna. El Owner resolvió por su cuenta,
y su resolución es **`O21`**, del **2026-09-01**, cuyo texto íntegro vive en la **SEDE
CANÓNICA**. Su bloque propio es **§15.8** del documento 11.

| # | decisión | qué revisa | por qué, y qué alternativa se descartó | cómo se revierte |
|---|---|---|---|---|
| D110 | **PROPAGACIÓN DE `O21`. No es una elección de F4c: es la materialización de una resolución del Owner, y se declara DERIVADA. No sustituye a `O21`.** (i) **`C-L.5` gana REGLA DETERMINISTA**: cumplidas sus seis condiciones sobre un gate válido, el adjudicador **DEBE** declarar `C-L.5 CERTIFICADA PARA ESTE GATE`; fallada cualquiera, **DEBE** declarar `C-L.5 ABIERTA` **nombrando la condición incumplida**. Deja de ser un acto discrecional. (ii) **Las dos declaraciones se SEPARAN**: cobertura y suficiencia arquitectónica son preguntas distintas, y un gate puede emitir `C-L.5 CERTIFICADA` e `INSUFICIENTE PARA F5` a la vez **sin contradicción**. (iii) **Un adjudicador NO puede negarse a certificar cobertura por haber encontrado otros defectos.** (iv) **La certificación queda LIGADA a una tupla exacta** —tree SHA candidato, commit candidato, SHA del manifiesto de asignación, SHA de los manifiestos de lectura, identificador del gate e identidad del adjudicador— y **NO se transfiere** a otra candidata ni a otro gate. (v) **Sobre el documento 30 no se inventa ningún acto**: se registra que las condiciones estaban satisfechas y que el acto no se emitió por ausencia de una regla entonces vigente, y `O21` sólo rige hacia adelante | **`O21`**, que es su única fuente. No revisa ninguna decisión de F4c y **no renumera nada**: `D1`–`D109` y `O1`–`O20` intactas. Tampoco revisa las SEIS condiciones de `C-L.5`, que siguen siendo las del documento 19, `O-04` y `P-08` | la AMBIGÜEDAD que el gate del documento 30 dejó a la vista: la regla de cierre de `C-L.5` es una EXCLUSIÓN de una sola dirección —dice cuándo la cobertura queda excluida, no cuándo queda certificada—, con lo que cumplir las seis condiciones no obligaba a nada. **Dos adjudicadores consecutivos actuaron de forma opuesta sin incumplir norma alguna**: `FF` certificó mientras devolvía insuficiencia, `HH` se negó por devolver insuficiencia. **Las alternativas NO las descartó F4**: las ponderó el Owner | retirar `O21` devuelve `C-L.5` a un estado que puede oscilar indefinidamente sin que nadie falte a ninguna norma, que es lo que el documento 30 midió |
...
| D112 | **PROPAGACIÓN DE `O23`. No es una elección de `F5`: es la materialización de una resolución del Owner, y se declara DERIVADA. No sustituye a `O23`, y una paráfrasis NUNCA amplía el texto canónico.** (i) **La sección `(g)` se adopta como NORMA BREVE**, con las materias que sus fuentes le reservaron, y su detalle mecánico baja a un CONTRATO DERIVADO que `F6` construye. (ii) **La raíz externa de confianza es materia de `(g)`**, con contrato derivado propio para `F6`, y su autoridad NO puede depender del árbol que verifica. (iii) **La reconciliación pendiente se sostiene en un REGISTRO OPERATIVO AUXILIAR DURABLE**, separado del estado canónico y del diario canónico: son TRES materias y no se colapsan. (iv) **El gate constitucional de arranque se CONSERVA**, y el circuito de instalación queda SUBORDINADO a él. (v) **Se reconoce la TERCERA VÍA de nacimiento del trabajo** —apertura automática por política aprobada—, que NO elude el gate constitucional. (vi) **Verificación entra en la ruta de auditoría como productora del dictamen**, y dominio, seguridad y diseño en la puesta en marcha de un producto nuevo. (vii) **Donde una tabla nombraba un método, pasa a nombrar la capacidad competente.** (viii) **Manda la grafía con tilde de la fuente aprobada** para los dos identificadores. (ix) **El mapa documental se satisface DERIVADO.** (x) **`F5` NO queda cerrada**: sólo se cierra por acto posterior y expreso del Owner | **`O23`**, que es su única fuente. No revisa ninguna resolución anterior y **no renumera nada**: `D1`–`D111` y `O1`–`O22` conservan íntegro su texto | las quince decisiones del paquete de `F5` —`D-01`–`D-10` y `R-01`–`R-05`—, que hasta `O23` no tenían acto |

### `D113` · la propagación de `O24` — DERIVADA de una resolución del Owner, NO elegida por `F6`

**Epígrafe propio por la regla de arriba.** **Origen:** `O23` §11 reservó el cierre de `F5` a
un **acto posterior y expreso del Owner**, sobre cuatro cosas demostradas. El Owner lo emitió:
**`O24`**, del **2026-09-02**, cuyo texto íntegro vive en la **SEDE CANÓNICA** y **no se copia
aquí**.

```bash
awk '/^# /{p = ($0 ~ /^# `O24`/)} p' docs/owner/ADS-OWNER-RESOLUCIONES.md
```

**Nada resolutivo anterior se toca:** `O17`–`O23` conservan íntegro su texto —comprobado byte
a byte contra el commit que introdujo cada una— y `D1`–`D112` el suyo. **Ningún hallazgo se
declara SUPERADO por esta propagación**, y ningún contrato de `F6` se declara implementado por
ella: eso lo demuestra la evidencia ejecutable, no esta fila.

| # | decisión | qué revisa | por qué, y qué alternativa se descartó | cómo se revierte |
|---|---|---|---|---|
| D113 | **PROPAGACIÓN DE `O24`. No es una elección de `F6`: es la materialización de una resolución del Owner, y se declara DERIVADA. No sustituye a `O24`, y una paráfrasis NUNCA amplía el texto canónico.** (i) **`F5` queda CERRADA** por acto competente, sobre las cuatro condiciones de `O23` §11 demostradas. (ii) **`F6` queda INICIADA**, y su alcance es implementar **y certificar** los contratos aprobados por `F5`, respetando las invariantes de `(g)`. (iii) **Las decisiones sobre rutas, formatos, serialización, algoritmos, bloqueos, herramientas, despliegue y mecanismos internos pertenecen a `F6`** cuando la norma las dejó expresamente al contrato derivado, **y no pueden rebajar** atomicidad, durabilidad, integridad, recuperación, auditabilidad, independencia de la raíz externa ni compatibilidad o migración explícita. (iv) **La suficiencia de `F6` se demuestra con implementación ejecutable y pruebas reproducibles**: un documento, un contrato o un resultado simulado **no equivale** a implementación ni a certificación. (v) **PesquerApp sigue BLOQUEADA**, sin MVP, piloto desechable, adopción parcial, integración anticipada ni ejecución productiva. (vi) **`O24` no reabre `F4c` ni `F5`, no declara `F6` completada y no declara implementado o certificado ningún contrato antes de su evidencia ejecutable** | **`O24`**, que es su única fuente. Revisa el ESTADO de `F5` y de `F6` —de PENDIENTE DE CIERRE a CERRADA, y de NO INICIADA a INICIADA— y **no renumera nada**: `D1`–`D112` y `O1`–`O23` conservan íntegro su texto | el cierre de `F5` estaba reservado al Owner desde `O23` §11, y **ninguna otra vía existía**: ni un gate, ni una validación en verde, ni una declaración del coordinador. La alternativa —seguir acumulando comprobación documental sobre una fase técnicamente completa— la descartó el Owner expresamente al ordenar que el resultado principal pase a ser código y pruebas | retirar `O24` devuelve `F5` a PENDIENTE DEL ACTO DE CIERRE y `F6` a NO INICIADA, y deja sin acto habilitante todo lo construido bajo ella |

### `D114` · la propagación de `O25` — DERIVADA de una resolución del Owner, NO elegida por `F6`

**Epígrafe propio por la regla de arriba.** **Origen:** `FD-1` estaba registrada como la
única decisión del Owner que quedaba viva tras el primer corte de `F6`: `O23` §3 dijo a QUÉ
contrato pertenece la materia de claves, pero **no dijo quién es el titular ni quién
custodia**. El Owner lo resuelve: **`O25`**, del **2026-09-02**, cuyo texto íntegro vive en
la **SEDE CANÓNICA** y **no se copia aquí**.

```bash
awk '/^# /{p = ($0 ~ /^# `O25`/)} p' docs/owner/ADS-OWNER-RESOLUCIONES.md
```

**Nada resolutivo anterior se toca:** `O17`–`O24` conservan íntegro su texto —comprobado
como CADENA: cada versión de la sede es prefijo exacto de la siguiente desde su commit de
nacimiento— y `D1`–`D113` el suyo. **`O25` no declara implementada ni certificada la raíz
externa**, y esta fila tampoco.
...
### `D116` · la propagación de `O27` — DERIVADA de una resolución del Owner, NO elegida por `F6`

**Epígrafe propio por la regla de arriba.** **Origen:** el gate del 2026-09-04 —registrado en
`docs/f6/03-GATE-DE-CERTIFICACION-FINAL-20260904.md`— dejó tres cuestiones que **sólo el Owner
podía resolver**, porque las tres tocan la sede append-only o la forma de un acto ya emitido:
`ADJ-O2`, la omisión de la línea «FIN LITERAL DE `O26`.»; `ADJ-O1`, la ausencia de los campos
de procedencia y relaciones de revisión en `O23`–`O26`; y `ADJ-B3`, que el guardián append-only
sólo protege el prefijo del nacimiento. El Owner las aclara: **`O27`**, del **2026-09-04**,
cuyo texto íntegro vive en la **SEDE CANÓNICA** y **no se copia aquí**.

```bash
awk '/^# /{p = ($0 ~ /^# `O27`/)} p' docs/owner/ADS-OWNER-RESOLUCIONES.md
```

**Nada resolutivo anterior se toca:** `O17`–`O26` conservan íntegro su texto —comprobado como
CADENA sobre las OCHO versiones históricas de la sede, cada una prefijo exacto de la siguiente
desde el commit de nacimiento `1d3b5d41434bb3bcd0b3323c4e6d56af5d7676e8`, y bloque a bloque
byte a byte— y `D1`–`D115` el suyo. **`O27` es una ACLARACIÓN, no una revisión**: no reabre
ninguna resolución y no altera el alcance de `O26`.

**`O27` NO CERTIFICA NADA.** Lo dice de sí misma en su §6, y esta fila tampoco.

| # | decisión | qué revisa | por qué, y qué alternativa se descartó | cómo se revierte |
|---|---|---|---|---|
| D116 | **PROPAGACIÓN DE `O27`. No es una elección de `F6`: es la materialización de una aclaración del Owner, y se declara DERIVADA. No sustituye a `O27`, y una paráfrasis NUNCA amplía el texto canónico.** (i) **La línea «FIN LITERAL DE `O26`.» era un DELIMITADOR EXTERNO del encargo y no texto resolutivo**: su omisión fue CORRECTA y **no debe repararse añadiéndola ahora**. (ii) **Los campos de procedencia y relaciones de revisión son exigibles PROSPECTIVAMENTE** desde la resolución que estableció esa forma, y **no se insertan retroactivamente** en `O23`–`O26`, porque hacerlo rompería su literalidad, el carácter append-only de la sede y el digest del acto ya emitido; **su ausencia no las invalida**. (iii) **La protección append-only debe alcanzar a CADA resolución cerrada ya publicada, no sólo al prefijo del nacimiento**: una comprobación que permita borrar resoluciones posteriores y sustituirlas por otro texto **NO satisface la propiedad**, y la implementación deberá derivar las entradas cerradas, conservar cada una byte a byte y permitir únicamente añadir una entrada nueva y completa al final. (iv) **Los documentos del gate NO forman parte de la candidata**: toda validación del producto se ejecuta sobre un checkout CONGELADO del SHA de la candidata, y el manifiesto, los dictámenes y el documento del gate viven en la rama del gate **sin poder alterar la línea base auditada**. (v) **Un gate NO puede llegar a adjudicación mientras algún revisor tenga `ASIGNADO − LEÍDO` distinto del conjunto vacío**: un lote incompleto no se cierra, no se sustituye con búsquedas y no se compensa con lo leído por otro agente. (vi) **No certifica `F6`, no corrige ningún hallazgo técnico, no declara satisfecho `B3`, no inicia PesquerApp y no rebaja ninguna obligación**, y conserva íntegra la aceptación condicionada de `O26` | **`O27`**, que es su única fuente. Revisa la INTERPRETACIÓN de tres cuestiones que el gate del 2026-09-04 dejó abiertas —`ADJ-O1`, `ADJ-O2` y el criterio de `ADJ-B3`— y **no renumera nada**: `D1`–`D115` y `O1`–`O26` conservan íntegro su texto. **No revisa el estado de `F6`, que sigue `ABIERTA · INICIADA · EN CURSO`, ni el de PesquerApp, que sigue `BLOQUEADA`** | `ADJ-O1` y `ADJ-O2` eran INSUBSANABLES por quien corrige: reparar la forma de una entrada ya cerrada exige editar la sede append-only, y sólo el Owner puede decir si la forma se exige hacia atrás o hacia delante. La alternativa —insertar los campos y la línea faltante dentro de `O23`–`O26`— **la descartó el Owner expresamente** en `O27` §1 y §2: rompería la literalidad del texto y el digest del acto emitido, que es exactamente lo que la sede existe para conservar. Y `O27` §3 **no rebaja `ADJ-B3`: lo endurece**, porque escribe como norma lo que el gate encontró que faltaba | retirar `O27` devuelve `ADJ-O1` y `ADJ-O2` a defectos abiertos sin autoridad que los resuelva, deja el criterio de append-only sin norma expresa, y devuelve la cobertura de gate a regla de manifiesto en vez de regla del Owner |
...
> El bloque de abajo se presentaba con aspecto de literal y **convierte dos «debe» de la
> sede en indicativo**, con lo que DEBILITA el texto canónico. `O19` nació de una
> proyección que decía MENOS de lo que el Owner había resuelto, y ésta es la misma clase en
> la dirección contraria a la que se vigilaba. **No se reescribe la sede y no se amplía
...
### `O21` · `C-L.5` DEJA DE SER UN ACTO DISCRECIONAL — 2026-09-01

> **ESTA ENTRADA ES UNA PROYECCIÓN DERIVADA. LA AUTORIDAD CANÓNICA ESTÁ EN
> [`docs/owner/ADS-OWNER-RESOLUCIONES.md`](../owner/ADS-OWNER-RESOLUCIONES.md), `O21`.**
...
**Procedencia: decisión expresa del Owner del 2026-09-01**, tomada tras el GATE
ARQUITECTÓNICO FINAL —documento 30—. **No responde a una pregunta que ningún gate le
elevara**: los nueve declararon que ninguna decisión volvía a él. La toma por su cuenta.

**El vacío que resuelve, y no es culpa de nadie: `C-L.5` decía cuándo la cobertura queda
EXCLUIDA y no decía cuándo queda CERTIFICADA.** Su regla de cierre es una exclusión de una
sola dirección —«cualquier fuente ASIGNADA pero NO LEÍDA impide la suficiencia»—, de modo que
cumplir las seis condiciones **no obligaba a nada**. Con eso, dos adjudicadores consecutivos
actuaron de forma OPUESTA sin incumplir norma alguna: `FF` certificó la cobertura **mientras**
devolvía insuficiencia, acotando que una cosa no es la otra; `HH` se negó a certificarla
**por** devolver insuficiencia. Los dos podían. `O21` convierte una decisión en una
comprobación.

| # | decisión | qué revisa | lo resuelto |
|---|---|---|---|
| O21 | **`C-L.5` CERTIFICA COBERTURA Y NADA MÁS, ES INDEPENDIENTE DEL VEREDICTO, Y NO ES DISCRECIONAL**: **PARA UN GATE VÁLIDO** —precondición literal de la sede, y sin ella la regla no se aplica—, cumplidas las seis condiciones, el adjudicador **DEBE** declarar `C-L.5 CERTIFICADA PARA ESTE GATE`; fallada cualquiera, **DEBE** declarar `C-L.5 ABIERTA` **nombrando la condición incumplida** | **la SEMÁNTICA de `C-L.5`.** NO revisa sus seis condiciones —son las que ya estaban escritas por el documento 19, `O-04` y `P-08`— y **`O17`–`O20` conservan íntegramente su texto** | los diez puntos de la sede: qué certifica, independencia del veredicto, no discrecionalidad, las seis condiciones, la tupla de ligadura, la no transferencia, la coexistencia de las dos declaraciones, la prohibición de negarse, el tratamiento del documento 30 y lo que `O21` no hace |

**LO QUE `O21` NO HACE** —lectura de este registro, **NO literal de la sede**, y por eso
ninguna valla la encierra—:

- **no toca las seis condiciones**: son exactamente las que `C-L.5` ya exigía;
- **no declara suficiente a `F4c`** ni corrige ninguno de los dieciséis del documento 30;
- **no autoriza** `F5`, `F6` ni PesquerApp;
- **no reescribe** `O17`, `O18`, `O19` ni `O20`;
- **no se aplica hacia atrás**: sobre el documento 30 se registra el hecho —condiciones
