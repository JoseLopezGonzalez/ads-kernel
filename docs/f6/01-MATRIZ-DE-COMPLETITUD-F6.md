...
| **`CONTRATO 2`** · ampliar `T152` a toda sede que publique versión | `11-ARQ` §19: «que ninguna sede VIVA publique una versión o un recuento obsoleto, y que el alcance de `T152` sea derivado» | `PLT` | **abierta**: `T152` recorría **sólo `README.md` y `START_HERE.md`**, y por eso pasaba en verde mientras `kernel/operativo/00-INDICE.md` declaraba `KERNEL.md` «1.3.0» siendo 1.5.0 | ninguna sobre el alcance | **A** · INTERNO | el alcance se DESCUBRE por barrido y la versión vigente se resuelve contra `kernel/VERSIONES.md`, su sede única; los remedios se REPORTAN por clase de sede sin decidirlos | **IMPLEMENTADO_Y_PROBADO** · `T152` y `T272` · sabotajes `N272`, `N272b`. **La PRUEBA POSITIVA prescrita SE DIO y consta: al ampliarse el alcance, `T152` SE PUSO EN ROJO sobre `kernel/operativo/00-INDICE.md`**, que declaraba `KERNEL.md` «1.3.0» siendo 1.5.0; esa sede se corrigió. **Hoy `T152` sale `SUPERADA`** y `T272` publica, sin decidir su remedio, las sedes de la capa documental fuera de `F6` que siguen publicando una versión obsoleta. El estado vigente y esa lista no se escriben aquí: se leen de `evidencia/versiones-salida.txt` |
| **`D104`** · instanciar los pares `<CAP>:revision` | `11-ARQ` §19, ficha `D104`: «**F6 MATERIALIZA; no elige la forma**», con el error `composicion-incompleta` que «no es un aviso: impide el cierre del gate de composición» | `SIS` | **abierta**: **cero** instancias de `:revision` en todo `kernel/operativo/` y **cero** validadores que lo comprobaran | ninguna | **A** · INTERNO | se materializan los **NUEVE** pares del catálogo en `recorrido/01-PROCESOS.md`, con su herencia y su posición; se añaden a `circuitos/` las dos instancias de handoff de la ENTREGA DE VUELTA; y el gate de composición se construye entero: cuatro vías, discriminante estructural, ancla en sus dos ramas, regla por item, los tres repartos y un censo de **20 fixtures** contrastado contra su sede | **CERRADA** · `T273`–`T276` en verde · sabotajes `N273`…`N273f`, `N275`, `N276`. **Los CUATRO pares de `DOM` SÍ están.** Primero fueron ininstanciables —`esquemas/proceso.yaml` no admitía la variante `DOM:revision`—, y el bloqueo lo resolvió el coordinador el 2026-09-04 con la razón escrita en el propio esquema: `D104` EXIGE la variante por nombre en sus casos positivos de la vía 4 y reserva a `F6` **materializar**, no elegir la forma, luego ampliar el conjunto es materialización. Y el sabotaje `N240d`, que medía «una variante que el esquema no declara» mutando precisamente a `DOM:revision`, se REAPUNTÓ a `DOM:auditoria`: sin eso, ampliar el esquema lo habría dejado decorativo EN SILENCIO |
...
> con los DOS dictámenes y la adjudicación ÍNTEGROS. **Y la clasificación de sus veintiséis
