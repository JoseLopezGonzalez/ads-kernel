...
| **`F6-H` · hallazgos externos con propietario y fase `F6`** | `11-ARQ` §19 | **PARCIAL** · las OCHO filas `F-nn` con fase `F6` quedan cerradas —`F-01` mitad de `F6`, `F-02`, `F-04`, `F-05` (i), `F-06`, `F-07`, `F-10` y `F-11`—, cada una con prueba propia `T240`–`T246` y con catorce sabotajes que la ponen ROJA. Y de las CUATRO obligaciones de fase `F6` que la misma sección declara y que el inventario anterior omitía: `CONTRATO 1` **CERRADO** (`T151`, `T270`, sabotajes `N270`–`N270c`) · `CONTRATO 1bis` **CERRADO** (`T271`, `N271`) · `CONTRATO 2` **CERRADO** (`T152`, `T272`, `N272`, `N272b`) · `D104` **PARCIAL**: materializa CINCO de sus NUEVE pares `<CAP>:revision` y su gate de composición NO cierra. `CONTRATO 3` es `F6-I` y está cerrado. El inventario uno a uno vive en la [matriz de completitud](01-MATRIZ-DE-COMPLETITUD-F6.md) §2, y su universo lo deriva `derivar-universo-obligatorio.py --obligaciones` | **`F-08` NO es de `F6`**: su fase es `F5` y su propietario es el Owner —`11-ARQ` §19—, y `O23` §10 ya emitió su nota de vigencia. **Los CUATRO pares `DOM:revision` de `D104`**: `esquemas/proceso.yaml` no admite esa variante y ampliar el conjunto es un ACTO NORMATIVO cuya sede es el esquema |
