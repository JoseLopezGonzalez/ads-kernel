...
> **DERIVADO, y ahora se dice DE QUÉ y CÓMO, que es lo que faltaba.** Declararse derivado y
> publicar estado propio al lado es tener dos verdades con una de ellas etiquetada. La
> distinción construido/diseñado **no se copia aquí**: su sede es `04-CONTRATOS-TECNICOS.md`
> §1, y **esa sección era FALSA en los dos sentidos hasta el 2026-09-04** —declaraba NO
> CONSTRUIDO el ciclo de `§7.2`, la materialización de equipos, los gates de capa, los
> macrocircuitos, la prueba de humo en sesión nueva, la raíz externa y los dos últimos
> puntos del verificador de admisión, todos con evidencia publicada en este repositorio, y
> se contradecía con su propia §1.1—. Se corrigió allí, en su sede, y **este documento no
> reproduce el arreglo**: remite.
>
> Lo que este documento sí publica es el reparto contrato a contrato de `F6`, y **el
> universo contra el que se mide no se escribe**: se deriva, y con él las tres restas que
> `O26` §5 exige vacías antes de certificar.
>
> ```bash
...
| **`F6-H` · hallazgos externos con propietario y fase `F6`** | `11-ARQ` §19 | **PARCIAL** · las OCHO filas `F-nn` con fase `F6` quedan cerradas —`F-01` mitad de `F6`, `F-02`, `F-04`, `F-05` (i), `F-06`, `F-07`, `F-10` y `F-11`—, cada una con prueba propia `T240`–`T246` y con catorce sabotajes que la ponen ROJA. Y de las CUATRO obligaciones de fase `F6` que la misma sección declara y que el inventario anterior omitía: `CONTRATO 1` **CERRADO** (`T151`, `T270`, sabotajes `N270`–`N270c`) · `CONTRATO 1bis` **CERRADO** (`T271`, `N271`) · `CONTRATO 2` **CERRADO** (`T152`, `T272`, `N272`, `N272b`) · `D104` **CERRADO**: materializa los NUEVE pares `<CAP>:revision` y su gate de composición cierra (`T273`–`T276` en verde, con nueve sabotajes). `CONTRATO 3` es `F6-I` y está cerrado. El inventario uno a uno vive en la [matriz de completitud](01-MATRIZ-DE-COMPLETITUD-F6.md) §2, y su universo lo deriva `derivar-universo-obligatorio.py --obligaciones` | **`F-08` NO es de `F6`**: su fase es `F5` y su propietario es el Owner —`11-ARQ` §19—, y `O23` §10 ya emitió su nota de vigencia. **Los CUATRO pares `DOM:revision` de `D104` ya NO son un pendiente**: eran ininstanciables porque `esquemas/proceso.yaml` no admitía la variante, y el acto normativo que la admite lo emitió `F6` el 2026-09-04 —`D104` la exige por nombre y reserva a `F6` MATERIALIZAR—, con el sabotaje `N240d` reapuntado para que no perdiera poder de discriminación |
...
| **la AUTORIDAD de cada documento del Owner se declara en `exclusiones.yaml` y se DERIVA de su clase canónica** | escribir el campo dentro de cada fichero de `docs/owner/`, que es lo que la fila `F-07` pide literalmente · llevar al Owner la elección del valor | dos de los tres ficheros son `AUTORIDAD_SUPERIOR` en `FUENTES-CANONICAS.yml`, y la condición de contenido de esa clase es **append-only contra el nacimiento**: insertar una línea de metadatos en su cabecera es reescribir lo publicado, y `V6-12` lo da en ROJO. **Y aquí esta celda decía de más: decía «sólo un resultado es conforme, así que no hay decisión del Owner que tomar».** La auditoría independiente lo MIDIÓ ejecutando el verificador sobre copias y encontró **al menos TRES resultados conformes**, no uno —el campo al FINAL de los dos ficheros append-only sale `INDETERMINADO` sin hallazgo, y el tercer fichero es `NO_APLICABLE_A_IMPLEMENTACION` y admite el campo dentro—; la medición y sus tres casos están en [`01-MATRIZ-DE-COMPLETITUD-F6.md`](01-MATRIZ-DE-COMPLETITUD-F6.md), fila `F-07`, que es su sede. **Lo que sostiene la decisión es otra cosa, y se sostiene sola:** los tres resultados conformes declararían el MISMO valor, derivado de la misma clase canónica, y **ninguno altera comportamiento normativo**; el criterio para parar exige que dos conformes lo alteren, y aquí no lo alteran. Es materia de MECANISMO, que `O24` §2 reconoce a `F6`. Y el valor tampoco se elige: `AUTORIDAD_SUPERIOR → aprobada` y `NO_APLICABLE_A_IMPLEMENTACION → trabajo`, con ROJO si alguien escribe otro. **No se ha alterado un solo byte de ningún documento del Owner** |
