...
|---|---|---|
| **`G-01`** | **CERRADO** · el lector del manifiesto valida el rango contra las líneas reales del árbol y falla cerrado nombrando el defecto que hundió el gate del 2026-09-05 | `comprobar-cobertura-de-gate.py --autopruebas`, cuyo cardinal de controles NO se copia aquí: lo publica la última línea de la propia corrida. Lo que esta fila afirma es el invariante: **0 sin detectar** |
...
| **`G-03`** | **CERRADO** por `A1` · la guarda cambia el MOMENTO —`-I -S -E` antes de que `sitecustomize` exista— y alcanza a **TODO** punto ejecutable del inventario derivado —sin una sola exención—, con mecanismo idéntico byte a byte. **El cardinal NO se copia aquí**: lo deriva `T380` del árbol con `inventariar_el_arbol()`, que es la lección de `HALL.10` | `T380`–`T397` · `test_integridad_y_evidencia.py` |
