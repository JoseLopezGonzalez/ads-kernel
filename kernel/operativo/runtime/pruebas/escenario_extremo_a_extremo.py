...
def main():
    ...
    try:
        ...
        escenario.limpiar()
    # `D-02` · el escenario sale NOMBRADO con su veredicto. El recuento de pasos de arriba
    # dice cuántos cumplieron, no si `T180` pasó: quien lee la evidencia tenía que atar el
    # título de la cabecera con el recuento del final por su cuenta. Un escenario sin
    # veredicto nominal está ejecutado, no probado.
    escenario.lineas.append("")
    escenario.lineas.append(
        "T180  %s  F6 · corte vertical 1 · quince pasos del §12.1 del contrato"
        % ("SUPERADA" if escenario.codigo_de_salida() == 0 and recuperable else "FALLIDA "))
