...
class CanalGit:
    ...
    def actualizar_ref(self, ref, nuevo, viejo_esperado, *, protegidas=()):
        ...
        if codigo != 0:
            ...
            raise GitFallo(
                "la comparación e intercambio sobre la ref fue rechazada: " + detalle,
                ruta=ref,
            )
    ...
    def retirar_rama(self, ref, viejo_esperado, *, protegidas=()):
        ...
        if codigo != 0:
            raise GitFallo(
                "la retirada de la rama fue rechazada: "
...
#   · el borrado de una ref protegida
#   · toda actualizacion que no sea fast-forward
set -eu
fase="${1:-}"
...
while read -r viejo nuevo ref; do
    ...
    [ "$viejo" = "$nulo" ] && continue
    if ! git merge-base --is-ancestor "$viejo" "$nuevo" 2>/dev/null; then
